"""确认 Pyright 配置与 uv 当前解释器使用同一个项目环境。"""
...
def require_compatible_environment(
    *,
    ...
    """拒绝 uv 与活动 Pyright 配置指向不同环境时产生误导性的结果。

    本检查只读取 `pyrightconfig.json` 及其继承链，不生成或改写使用者配置。
    """

    config_path = active_config(project_root=app_root, pyproject=pyproject)
    ...
    raise SystemExit(
        "Pyright configuration expects the project environment at "
...
def main() -> int:
    """验证当前 uv 解释器与 service-app Pyright 配置一致。"""
