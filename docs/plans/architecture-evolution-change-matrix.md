...
|---|---|---|---|---|---|---|---|---|---|---|
| 17 | `architecture-governance-baseline`（非 OpenSpec 文档批次） | 冻结工程原则、贡献规则、living plan、handoff 与矩阵；同一 v1.20 修订补充 Phase 18.1 规划 | 无 | 工程原则、贡献流程、OpenSpec/Agent Pack 门禁语言 | 文档互链、事实优先级、状态语义、handoff 可恢复 | `docs/plans/architecture-evolution-*.md`；`docs/engineering-principles*.md`；`CONTRIBUTING*.md` 按本轮独立 owner 分配；Product Spec/DEV/API Contract 由主控整合 | 可做认知并行；写入必须按文件独占 | 当前工作树即可；无需为纯文档另开实现 worktree | 4-8 小时；Phase 18.1 规划补充另计本轮执行 | `初始基线已审查并本地提交；同提交规划补充须 final fresh review 后 amend`；不代表 checker 或模型能力已实现 |
| 17.1 | `acceptance-criteria-identity-uniqueness` | 消除重复 `AC-070` 对 Spec、矩阵和 policy/checker 语义的覆盖，建立机械唯一性门禁和历史追溯 | Phase 17 文档冻结；用户授权 | AC identity grammar、Product Spec、acceptance matrix、required producer/test policy | 每个 live AC identity 唯一；两项原 `AC-070` 各自保留正确行为、producer、test 和 evidence；历史引用可追溯 | `Product-Spec.md`、`docs/acceptance-matrix.md`、`scripts/acceptance_matrix{,_policy}.py`、相关 contract tests；必要的 live docs 引用 | 串行；不与会修改 Product Spec/验收矩阵的 change 并行 | 单一 worktree；先 inventory 全部引用，禁止局部顺手重编号 | 2-5 小时 | `计划`；本轮只记录风险 |
...
1. Phase 17 治理文档已完成交叉核对、fresh 独立静态审查和 handoff 复原测试，并形成文档专用本地提交；Phase 18.1 规划按用户授权作为同一 v1.20 变更重新冻结、review 后 amend，精确提交状态以 Git 为准。
2. 保持 `openspec list --json` 为空，直到用户明确授权首个后续 active change，即 Phase 17.1 的验收标识治理；它不是生产行为实现。
3. 重复 `AC-070` 只允许通过独立 `acceptance-criteria-identity-uniqueness` 处理；先 inventory 再设计 identity 迁移，不顺手重编号、不重写历史归档。
4. identity change 完成验证、审查和归档且用户授权后，只创建 `controlled-real-model-runtime`，按本矩阵单 worktree 串行执行。
5. Phase 18 完成验证、fresh review、同步和归档且用户再次授权后，创建 `controlled-model-streaming`；先更新 API/event contract，再按本矩阵单 worktree实现有界 delta/capacity/settlement/recovery，不跳过到 Phase 19。
6. 在各自 proposal/design/tasks 中把对应矩阵行的依赖、共享接口、验收、文件所有权和非目标复制为可审查契约；strict + fresh review PASS 前不写实现。
