...
- Public seam: `ModelProvider`, `ModelStructuredProvider`, `ModelToolIntentProvider`, `ModelRequest`, `ModelResponse`, `StructuredOutputResult`, `ModelTurnResult`, `ModelRouter`, and `ModelInvocationService`; embeddings use `EmbeddingProvider` and a cache protocol.
- Action: implement providers in `agent_harness/adapters/models/`, bind them in the composition root, keep fake providers limited to explicit local/test routes, and cover usage/cost/latency and replay contracts. For structured results, bind a strict `output_schema` in the Agent `config.yaml`, then call `complete_structured(..., operation_key=..., repair_limit=0..2)` from a bound execution. Callers may narrow the repair limit but cannot submit a different schema. For tool intent, let the composition root build the catalog from the Agent `tool_allowlist` and the same `ToolRegistry`, then call `complete_tool_intent(..., tool_selection=...)`. It returns only `final_text` or a data-only `tool_intent`; a later Registry/Policy/HITL seam still owns tool execution.
- Controlled loop: a tool-enabled Agent must declare all five exact `model_tool_loop` maxima with no defaults. Resolve the run-bound `model_tool_loop` service and call `run(..., limits=ModelToolLoopLimitOverrides(...))`; overrides may only narrow maxima. Runtime owns the single linear model/Registry/Policy/HITL/tool/output-guard/Context sequence, emits existing CanonicalEvents, and pauses dangerous actions through the existing approval grant/lease state machine. Do not add a provider callback, parallel tool execution, an implicit retry, a second approval state machine, or a new HTTP route.
- Migration and recovery: upgrade the database to `0018_model_tool_loop_state` before running tool loops. One loop's model/tool/context/usage/terminal state advances only through repositories/UoWs, lease/fence, and version CAS. Determinate results replay exactly; only trusted `tool-handler-not-started-v1` proof permits takeover of an expired `claimed` row, while every `executing` or unknown result enters `needs_review`. The first v1 evidence and the `model-tool-loop-v1` marker commit in one transaction, and the raised marker cannot be cleared; an old `0017` binary must reject the new database before any side effect. Never delete evidence to force a downgrade, reset a deadline/budget, or retry an unknown tool effect automatically.
- Forbidden: vendor SDK imports in business Agents; provider/Pydantic objects in DTOs; skipping identity, budget, policy, or approval before invocation; inventing zero cost when cost is unavailable; adapter-owned repair/retry; raw invalid candidates in evidence; text/fake fallback; or interpreting structured JSON as a tool call. A tool-intent adapter never receives registry/handler/policy/approval callbacks, executes a tool, or expands the Agent allowlist; it must fail closed when its SDK cannot prove zero-execution observation.
- Validate: `make test`, `make smoke-local`; run `make smoke-service` only for real cross-process persistence.
- Evidence: `tests/contracts/test_agent_registry_router_model_contracts.py`, `tests/contracts/test_model_usage_invocation_contracts.py`, `tests/contracts/test_model_usage_runtime_composition_contracts.py`, `tests/contracts/test_provider_neutral_structured_public_seam_contracts.py`, `tests/contracts/test_provider_neutral_tool_catalog_contracts.py`, `tests/contracts/test_tool_intent_usage_settlement_contracts.py`, `tests/contracts/test_policy_gated_model_tool_loop_public_seam_contracts.py`, `tests/contracts/test_policy_gated_model_tool_loop_event_contracts.py`, `tests/contracts/test_model_tool_loop_migration_contracts.py`, `tests/contracts/test_model_tool_handler_not_started_takeover_contracts.py`, `tests/contracts/test_model_tool_execution_unknown_needs_review_contracts.py`, `tests/contracts/test_model_tool_loop_postgresql_contracts.py`.
- Troubleshoot: inspect profile, model policy, deployment capability, Agent `output_schema_identity`, and `tool_allowlist` for routing failures. For duplicate usage, inspect the stable call ID, operation/replay identity, outbox, and settlement. For tool intent, also verify canonical catalog bytes, selection, and the provider's zero-execution observation capability. Fix Registry loading for `model.structured_schema_unknown`; keep reservations fenced and review `needs_review` instead of resending. Retain only redacted stable errors, never raw responses or invalid candidates.
