...
| uv CLI | root `pyproject.toml`, GitHub setup action, GitLab image | exactly `0.11.29`; local, both CI systems, release wrapper, and `uv publish` use the same version; `required-version` fails closed otherwise |
...
|---|---|---|---|
| uv | `0.11.29` | [uv 0.11.29 metadata](https://github.com/astral-sh/uv/blob/0.11.29/pyproject.toml), [`uv lock --check`](https://docs.astral.sh/uv/concepts/projects/sync/#checking-the-lockfile), and [build/publish guide](https://docs.astral.sh/uv/guides/package/); root `required-version`, GitHub setup, GitLab image, and release wrapper use the same version | workspace development uses root `tool.uv.sources`; publication compatibility uses `uv build --no-sources` and installation outside the workspace so metadata cannot depend on local workspace paths |
| python-semantic-release | `10.6.1` | [10.6.1 metadata](https://github.com/python-semantic-release/python-semantic-release/blob/v10.6.1/pyproject.toml) and [version command](https://python-semantic-release.readthedocs.io/en/latest/api/commands.html#semantic-release-version); only Conventional Commits parsing and next-SemVer calculation are reused, while repository wrappers own side-effect-free preview | exact pin in the root `release` dependency group and `uv.lock`; not a core/template runtime dependency; dry-run never invokes commit/tag/release-writing paths |
