...
- 公开 seam：`ModelProvider`、`ModelStructuredProvider`、`ModelToolIntentProvider`、`ModelRequest`、`ModelResponse`、`StructuredOutputResult`、`ModelTurnResult`、`ModelRouter`、`ModelInvocationService`；embedding 使用 `EmbeddingProvider` 与 cache protocol。
- 操作：在 `agent_harness/adapters/models/` 实现 provider；在 composition root 绑定；保持 fake provider 只用于显式 local/test route；为 usage/cost/latency 和重放补合同证据。业务 Agent 需要结构化结果时，在 `config.yaml` 绑定严格 `output_schema`，再从 bound execution 调用 `complete_structured(..., operation_key=..., repair_limit=0..2)`；调用方只能缩小 repair 上限，不能传入另一份 schema。需要工具意图时，由 composition root 从 Agent `tool_allowlist` 与同一个 `ToolRegistry` 构建 catalog，再调用 `complete_tool_intent(..., tool_selection=...)`；它只返回 `final_text` 或 data-only `tool_intent`，工具执行仍由后续 Registry/Policy/HITL seam 负责。
- 受控循环：支持工具意图的 Agent 必须声明 exact `model_tool_loop` 五项 maxima，且没有默认值。取得 run-bound `model_tool_loop` 服务后调用 `run(..., limits=ModelToolLoopLimitOverrides(...))`；override 只能缩小 maxima。Runtime 独占线性的 model/Registry/Policy/HITL/tool/output-guard/Context 顺序，复用既有 CanonicalEvent，并通过同一 approval grant/lease 状态机暂停危险动作。不得新增 provider callback、并行工具执行、隐式重试、第二审批状态机或 HTTP route。
- 迁移与恢复：运行工具循环前把数据库升级到 `0018_model_tool_loop_state`。同一 loop 的 model/tool/context/usage/terminal 只能通过 repository/UoW、lease/fence 和版本 CAS 推进；确定结果 exact replay，只有可信 `tool-handler-not-started-v1` 证明允许接管过期 `claimed`，任何 `executing` 或未知结果都进入 `needs_review`。首条 v1 evidence 与 `model-tool-loop-v1` marker 同事务提交，marker 提升后不可清零；旧 `0017` binary 必须在任何副作用前拒绝新库。不得删 evidence 强制降级、重置 deadline/预算或自动重试未知工具影响。
- 禁止：业务 agent import Pydantic AI 或 provider SDK；把 provider/Pydantic object 放进 DTO；在调用前跳过 identity、budget、policy 或 approval；用虚构的零成本替代 unavailable；在 adapter 内 repair/retry、把无效候选原文写入 evidence、把 structured 调用退回普通文本/fake，或把结构化 JSON 解释为工具调用。工具意图 adapter 不得接收 registry/handler/policy/approval callback、执行工具或扩张 Agent allowlist；SDK 无法证明零执行观察时必须 fail closed。
- 验证：`make test`、`make smoke-local`，涉及真实跨进程持久化再跑 `make smoke-service`。
- 证据：`tests/contracts/test_agent_registry_router_model_contracts.py`、`tests/contracts/test_model_usage_invocation_contracts.py`、`tests/contracts/test_model_usage_runtime_composition_contracts.py`、`tests/contracts/test_provider_neutral_structured_public_seam_contracts.py`、`tests/contracts/test_provider_neutral_tool_catalog_contracts.py`、`tests/contracts/test_tool_intent_usage_settlement_contracts.py`、`tests/contracts/test_policy_gated_model_tool_loop_public_seam_contracts.py`、`tests/contracts/test_policy_gated_model_tool_loop_event_contracts.py`、`tests/contracts/test_model_tool_loop_migration_contracts.py`、`tests/contracts/test_model_tool_handler_not_started_takeover_contracts.py`、`tests/contracts/test_model_tool_execution_unknown_needs_review_contracts.py`、`tests/contracts/test_model_tool_loop_postgresql_contracts.py`。
- 排障：route 失败先查 profile、model policy、deployment capability、Agent `output_schema_identity` 与 `tool_allowlist`；重复 usage 检查稳定 call id、operation/replay identity、outbox 和 settlement；tool intent 还要核对 canonical catalog bytes、selection 与 provider 的零执行观察能力。`model.structured_schema_unknown`先修 Registry 全量加载，`needs_review`保留 reservation 并人工核对，不重发 provider。Provider 失败只保留结构化脱敏错误，不记录 raw response/无效候选。
