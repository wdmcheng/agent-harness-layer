...
When an Agent needs the model to propose a tool intent, keep using the existing `tool_allowlist`; do not add a bypass configuration surface. Runtime builds the canonical catalog from that allowlist and the same `ToolRegistry`. With `complete_tool_intent(..., tool_selection=...)`, an omitted selection means the full ordered projection and an explicit empty selection gives the model no tools. The exact `ModelTurnResult` is only `final_text` or a data-only `tool_intent`; this step never executes a tool. Registry resolution, Policy/HITL, and pre-execution revalidation belong to the later execution seam. The currently locked Pydantic AI adapter fails closed when it cannot prove zero-execution observation; local contract tests use only an explicitly finite fake-provider script.

To let Runtime execute a controlled model tool loop, a tool-enabled Agent must also declare the exact `model_tool_loop` object in `config.yaml`: `max_turns`, `max_total_tokens`, nullable `max_total_cost_usd`, `max_tool_output_bytes`, and `max_duration_seconds` are all required and have no defaults. Obtain the run-bound `model_tool_loop` service from the execution context and call `run(request, operation_key=..., tool_selection=..., limits=...)`; `ModelToolLoopLimitOverrides` may only narrow the Agent maxima. Runtime alone advances model turn → Registry resolve → Policy/HITL → guarded tool result → untrusted `ContextAssembler` input → next model turn. A dangerous action returns the existing waiting approval and resumes through the existing grant/lease path; business code must not call `ToolRegistry.call()` directly, execute provider-native tools, retry a failed turn, or copy raw tool output into the prompt. Model, tool, context, and approval evidence use the existing `CanonicalEvent` stream. This public seam does not add an HTTP route.

Durable recovery requires the database at `0018_model_tool_loop_state`. For one `loop_id`, persisted identity, lease/fence, and version CAS serialize model turns, tool claims, approvals, context assembly, usage, and the terminal result. Determinate results replay exactly; only an expired `claimed` row with trusted `tool-handler-not-started-v1` proof may be re-leased. `executing`, an unknown commit result, digest drift, or an unknown event version transitions to `needs_review` and is never retried automatically. The first v1 evidence monotonically raises the `model-tool-loop-v1` marker in the same UoW; downgrade is forbidden once the marker is raised or an evidence scan is non-empty. A frozen `0017` binary/catalog must reject an `0018` database before any repository, worker, model, or tool side effect. Operators must not clear the marker, delete evidence to force a downgrade, reset a deadline/budget, or rewrite `needs_review` as success.
