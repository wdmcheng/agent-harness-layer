...
导航：[根 README](../README.zh-CN.md) · [service-app 模板](../templates/service-app/README.zh-CN.md) · [架构图说明](architecture/README.md) · [扩展指南](extension-guide.md) · [示例 Agent](../templates/service-app/docs/examples.md)

这份指南回答一个具体问题：拿到“五层 + 两翼”架构图后，怎样把一个业务想法落成可以运行、评测和观测的 Agent。

先纠正一个常见误解：五层不是五个都要由业务开发者重写的服务。模板已经提供接入层和运行时层的大部分能力；一个最小 Agent 主要实现引擎层的 schema/executor，通过配置声明模型与预算，按需增加工具层和基础设施适配，并让 Eval 与 Observability 两翼贯穿整个生命周期。
...
架构图中的 `Graph Nodes`、`GraphState`、复杂长期 memory 和独立 tool/model gateway 是目标扩展位，不是创建第一个 Agent 的前置。图中的 `@agent.tool Registry` 是概念性工具注册标签；当前项目的公共 seam 是 `ToolRegistry` 和类型化 descriptor/result DTO，没有可绕过 registry、policy 或 approval 的公共 decorator 语法糖。
...
这里的“声明式注册”就是实际便捷层：不需要在 FastAPI route 或 runtime composition 中为每个 Agent 手工接线，`AgentRegistry.load_from_directory()` 会统一校验配置、schema 和 executor。它不会跳过 runtime、policy 或 storage。
...
CLI 与 HTTP 最终进入同一 registry、runtime、DTO、storage 和 event seam。先用 CLI 验证业务 Agent，能减少把 schema/config 问题误判成 HTTP 问题。需要应用集成时再使用模板的 [HTTP API](../templates/service-app/README.zh-CN.md#http-api)。
...
详细 draft/approve/run 命令见[示例 Agent 指南](../templates/service-app/docs/examples.md#新增自己的-agent)，experiment 和 acceptance 见 [Eval 与 Observability 闭环](eval-observability-loop.md)。没有 approved case 时，稳定结果是 `no-approved-cases`，不是“评测通过”。
...
这就是“五层两翼”的实际用法：不是把七个区域都写一遍，而是明确哪些由框架负责、哪些由业务 Agent 实现、哪些能力只有需求出现时才开启，以及两翼如何证明行为可接受。
...
- 能通过 run ID 找到事件、usage、audit 与必要 artifact，secret 和 raw provider payload 不越界。
- 扩展只依赖公开 DTO/protocol/registry/facade/repository seam，不直接依赖内部实现或 vendor object。

更细的单项扩展方式见[扩展指南](extension-guide.md)，字段级 HTTP 合同见 [`API-Contract.md`](../API-Contract.md)，实际可运行范例见[四个示例 Agent](../templates/service-app/docs/examples.md)。
