...
[Phase 完成度判断]
    ...
    每步附当场跑的证据，最终收口必须有一份绑定冻结内容的完整四步验证证据。中间发生实质性代码、规则、契约、测试或配置改动时，旧 PASS 失效：先按影响矩阵补验；若改动涉及共享基础设施、依赖、配置，或无法证明影响边界隔离，四步重新来。审查后发生实质性 diff 必须从 Stage 1 重审；仅 .agents/.needs-review 的 clean 写入或删除不算实质改动。
