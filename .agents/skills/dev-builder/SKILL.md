...
[第一性原则]
    修改纪律：改代码前先评估影响范围，改完回归验证。改之前想清楚，不改坏已有功能。删或改一个被多处引用的东西（常量、导出、内置数据、组件、接口、文案）前，先 grep 全项目列出所有引用点、一次性同步清理，别只改定义留下游脱节。改完建立“改动 → 测试、编译、功能证据”影响矩阵，先运行失败回归和受影响门禁；涉及共享基础设施、依赖、配置，或无法证明影响边界隔离时，再运行全测试套件（typecheck + 单测 + e2e）。不得只靠 typecheck 判定通过——typecheck 过不代表单测/e2e 不挂。连带不止代码引用：删/改的东西若会被持久化（写进 db、本地存储、缓存、配置文件），还要管已经落库的存量数据——加启动迁移清理或做兼容，否则老用户的旧数据会变成「代码已删、却顽固显示、还删不掉」的僵尸。删持久化数据的改动配一条迁移测试。
    证据复用边界：验证是否需要重跑由改动的语义影响和证据适用范围决定。纯注释、docstring、维护说明，以及仅如实记录既有事实的状态文档或本地门禁 marker，在 diff 分类、注释纪律自检和必要的局部格式或语法检查证明未改变执行行为、规则或契约、验收结论后，可以复用原验证，不默认重跑全量测试；复用记录必须同时列出原冻结内容身份、当前 diff 和影响证明，不能把旧输出伪装成刚跑。改动触及代码行为、测试、配置、规则或契约、上游真相源、验收或完成声明、证据身份或真实性，或无法证明影响隔离时，旧证据失效，按影响矩阵补验，必要时重跑完整门禁。未运行项标记为 `NOT RUN` 并说明原因。
    术语与命名清理：批量清理前先写明目标词、替换映射和明确保留类，按“活动产品、测试、配置和维护文档”“不可改写的历史记录”“需要兼容的公开标识或 schema”分类。测试名和夹具属于活动维护面，除明确验证历史兼容性的夹具外，不能因为“只是测试”保留已退役术语；映射外的命名或可见性变化必须单独说明边界变化和引用证据，仅把函数移到辅助模块不构成去掉私有标记的理由。收尾用新旧词双向扫描和命名 diff 复核。
    ...
    验证即证据：完成声明必须在同一条消息里带上对当前内容仍有效的验证证据：刚跑的命令、输出和退出状态，或符合上述边界的证据复用记录。只说“之前编译过了”无效；没有证明旧证据为何仍适用于当前内容，就没有完成。
...
[输出风格]
    ...
    验证结果必须区分 `PASS`、代码或契约 `FAIL`、环境 `BLOCKED`、被中断和未运行。只有绑定当前内容、完整命令、环境前提和退出状态的结果才能下结论；成功只证明命令实际覆盖的接缝，不能外推为“没有其他问题”，失败也要先依据输出定位到代码、契约、依赖或环境再归因。
...
[开发规则清单]
    [代码规范]
        ...
        - 注释、docstring、测试说明、脚本说明、配置文件注释和配置示例说明按 [共享规则文件] 的通用注释纪律执行：默认使用项目主语言，不得无故把既有符合项目主语言的注释改成另一种语言；必要的标识符、协议字段、API 名称、CLI 参数、schema 字段、错误码和行业固定术语可保留英文。注释服务于维护意图、复杂控制流、数据形状、约束、边界、风险、设计取舍、兼容/迁移逻辑、测试夹具意图和项目规范要求；API、worker、迁移、并发、安全边界、测试夹具、公开 seam 等非显然函数和复杂代码块，必须说明维护意图、边界或验证目的；配置、脚本和示例里的非显然约束也要说明维护边界、默认值来源、兼容风险或验证目的；开发或整改触及注释纪律时，覆盖代码、docstring、测试说明、脚本说明、配置文件注释和配置示例说明，避免机械复述和 AI 式自述，但不能为了少写注释、压行数、套“最小修正/最小补充”措辞或过审而省掉维护者需要的说明
...
[Phase 执行流程]
    ...
    Phase 收口先完成公开 seam 红绿、受影响回归以及轻量静态/契约检查，冻结候选内容和证据身份，再由独立 reviewer 审查同一身份。
    不在每轮静态审查前重复运行项目定义的高成本全量门禁；静态审查所需证据不足时只补与发现项相关的聚焦验证。

[Phase 完成度判断]
    同步 Phase 或 change 状态时，必须分开记录“开发实现”“审查验证”“归档”三类事实：
        - 开发实现依据当前代码、对应 tasks 和可追溯提交判定；审查失效、待重审或待归档不等于实现未开发
        - 审查验证只对当次审核覆盖的代码和验证证据有效；执行语义、契约、验收结论或证据身份改变后旧 PASS 失效，但不反推开发实现状态；仅有符合证据复用边界的记录性改动时，须记录 diff 分类和影响证明，不机械重跑全量测试
    ...
    每步附对当前内容仍有效的证据，最终收口必须有一份绑定当前冻结内容的完整四步验证记录。复用旧证据时同时记录原冻结内容身份、当前 diff、影响证明和未运行项，不能把旧输出写成刚跑；中间发生实质性代码、规则、契约、测试或配置改动时，旧 PASS 失效：先按影响矩阵补验；若改动涉及共享基础设施、依赖、配置，或无法证明影响边界隔离，四步重新来。审查后发生实质性 diff 必须从 Stage 1 重审；仅符合证据复用边界的记录性改动，以及 .agents/.needs-review 的 clean 写入或删除，不算实质改动。
    同一冻结身份通过最终审查后，只运行一次项目定义的最终重型门禁，包括适用的全量测试、外部服务或性能 smoke、构建与许可证检查。
    重型门禁若证明实现缺陷并触发实质修订，必须重建聚焦证据、重新冻结并从 Stage 1 重审，随后重新执行最终重型门禁。
    性能失败先区分可复现代码回归与环境负载抖动，保留首次失败和隔离复验证据；不得抬高阈值，也不得把失败轮与成功轮合写成全绿。
