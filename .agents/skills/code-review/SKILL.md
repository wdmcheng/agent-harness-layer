...
[审查维度清单]
    ...
    - 变更契约：有 OpenSpec change 时，对照 proposal.md、specs/**/*.md、design.md、tasks.md 检查本次行为是否完整、是否有 scope creep；审查 OpenSpec 变更契约产物时把 `openspec validate` 只当格式和契约可解析证据，不当作审查结论
    ...
    - 代码质量：命名规范、无 any、单一职责、无重复、错误处理；按有效代码行审查文件规模，JS/TS/Python/Ruby/Go/Rust 超 300 行或 Java/C#/Kotlin/Swift 超 500 行要说明是否应拆分，前者超 500 行或后者超 1000 行默认判拆分问题；注释、空行、docstring/块注释说明行不计硬门槛，必要注释不能被行数规则反向删除；抽样代码、测试、脚本、配置文件和配置示例的注释、docstring、测试名、辅助脚本说明、配置说明和输出，发现注释语言无故偏离项目主语言、把既有符合项目主语言的注释改成另一种语言、缺少维护者理解意图/复杂控制流/数据形状/约束/边界/风险/设计取舍/兼容迁移/测试夹具意图/配置默认值来源/项目规范要求所需注释、遗漏配置说明、把说明削到不足以维护、或把 `Phase N` / `Phase 几` 这类开发阶段标签写进代码产物，或把 `P0/P1`、`Phase N` 当成 API-Contract.md 等长期契约文档的正文叙事时，按代码质量问题报告；若这些词是稳定标识、真实业务/领域概念、协议字段或用户可见产品语言，必须说明理由后放行
...
[输出报告]
    报告必须包含：Stage 1 结论、Stage 2 结论、Spec 轴、Standards 轴、编译结果。Stage 1 失败时明确"Stage 2 未执行"。
    多变更联合审查报告还必须分别列出每个 change 的结论，并给出唯一的联合 Stage 1/2 结论；任一跨 change HIGH/MEDIUM 问题都使整体 FAIL，不得用单项 PASS 或 `validate --all` 掩盖。
    ...
    发现项阻断边界：纯状态同步、非契约说明，以及不构成当前代码质量缺陷的命名或可维护性建议，在不影响功能、契约、安全、CI、artifact、权限和证据真实性时记为 deferred，不阻断 Stage 1/2，并在报告中写明判定依据；若文档本身是上游真相源、验收契约或错误宣称已完成，仍按实质 finding 阻断，不得以“只是文档问题”放行。
