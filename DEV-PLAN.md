...
- Phase 18: 受控真实文本模型运行时；上述最小纵向闭环已完成离线冻结验证、fresh 严格 `1+2`、主规格同步与归档；两次已授权 MiMo 入口未建立真实成功，默认 smoke 继续保持零调用。
...
- Phase 18.2: 受控跨 deployment/provider fallback；以有序 `(deployment_id, model_id)` route chain 扩展 Phase 18/18.1，只有前一候选可证明 not-started 时切换，逐候选独立凭据/endpoint/catalog/Bulkhead/预算/evidence，unknown 或已观察 delta 时停止。
- Phase 19: provider-neutral structured output；依赖 Phase 18.2 已稳定的 provider/invocation/result seam，单独设计 schema validation、unknown/repair/retry 和 replay evidence；不包含 structured streaming。
...
- AC-077 至 AC-084：受控真实文本模型非流式已完成 fresh `1+2` 实现审查、验证、主规格同步与归档，typed deployment、credential/endpoint/model catalog、只缩权冻结路由、async Pydantic AI adapter、共享预算/策略/审批/审计/evidence、composition 与默认离线门禁均已交付。真实 completion成功仍为 `hosted-unverified`，不得据离线 double、skip或 side-effect unknown勾成外部 PASS。
- AC-085 至 AC-088：当前实现已生产有界 provider-neutral delta/completed，使用 65 槽 stream reservation、跨块安全、unknown/partial fencing、恢复和 committed SSE/CLI reader。失效候选 `25cb9b53…` 的真实 PostgreSQL 5/5、全量 `1712 passed, 230 skipped`、eval 11/11、local/service smoke、build 与 license 均通过；AC-065 在该次全量运行中通过，阈值未放宽。授权 live stream 与 completion 分别因 `credential_missing`、`typed_preflight_missing` 返回 `hosted-unverified/provider_called=false`。Make 入口修复后，最终身份只在新 `1+2` 后重跑一次上述重型门禁。
- AC-090 至 AC-095：跨 deployment/provider fallback 的 typed route chain、跨候选 reservation transfer、保守 recovery、completion/streaming 围栏及双 deployment live artifact validator 已实现。AC-090～094 已有 SQLite、临时隔离 PostgreSQL 和公共 seam 合同；AC-095 因缺少双隔离真实凭据与受信 not-started fixture，只能保持零调用 `hosted-unverified/provider_called=false`，不能把离线 double 或 validator 正例冒充真实外部 PASS。
                                      ...
                                      -> Phase 17 Architecture governance / handoff baseline
                                        -> Phase 18 Controlled real text model runtime
                                          -> Phase 18.1 Provider-neutral text streaming
                                            -> Phase 18.2 Controlled multi-provider failover
...
并行规则：文档盘点、只读 blast-radius 和独立测试设计可以由 sub-agent 并行；生产实现只有在 change matrix 证明无顺序依赖、无共享接口、无共享验收且无文件所有权冲突时，才使用独立 worktree 并行。Phase 18、18.1、18.2、19、20 共享 model/route/result/runtime/event seam，必须按顺序串行；Phase 21 的候选 change 逐项重新证明独立性。
...
- 建立默认离线合同和单独 opt-in live smoke。缺本会话授权、显式 opt-in、隔离凭据或受信 endpoint 任一前置时，live smoke 唯一记录 hosted-unverified/CI skipped；四项齐全并授权后发生外部网络/provider 阻断才记录 external-blocked，不影响先验证 schema、路由、adapter 和 composition doubles。

**关键文件候选**：
- `packages/agent-harness/src/agent_harness/config/{schemas,settings,secret_files,model_endpoints,model_catalog}.py` - deployment、credential、受信 endpoint/model catalogs 与公开优先级。
...
- `packages/agent-harness/src/agent_harness/policy/engine.py`、`audit/service.py`、`approvals/{service,_continuation}.py`、`runtime/{services,executor,continuation,_run_continuation}.py`、`templates/service-app/app/runtime.py` - exact model policy、既有耐久审批 continuation、composition 与 provider 注册。
- `packages/agent-harness/src/agent_harness/scaffold_templates.py`、`_scaffold_support.py`、`tests/contracts/test_agent_scaffold_validation_atomicity_contracts.py` - 生成配置的 strict Agent policy、正式 Registry 校验与离线运行回归。
...
**当前状态**：以 `develop@5d7c018` 为实现冻结基线，品牌化 typed deployment/credential/endpoint/model catalog、deployment∩Agent∩request 只缩权、有序同 provider fallback、完整 `budget-tree-v2` route 恢复、async Pydantic AI/OpenAI-compatible adapter、可信 completion classifier、有界 retry/deadline/Bulkhead、既有 Policy/HITL、共享预算/usage evidence、lazy composition/lifecycle、显式 fake 与默认离线门禁均已落盘。最终全量的唯一旧授权夹具失败仍如实记录为 `1 failed, 1436 passed, 224 skipped`，其真实 lease 修复由 15 项受影响回归补证；后续发布前 settlement、受控 Router、approval lease、锁定 SDK 异常传播、encoded dot-segment 与职责拆分均按公共 seam 补证。fresh Reviewer 1、Reviewer 2、Reviewer 3 已在同一冻结身份上分别完成 Stage 1/2 PASS，均为 0 findings。三份 delta specs 已同步主规格，change 已归档到 `openspec/changes/archive/2026-07-29-controlled-real-model-runtime/` 并由本地提交 `ff0c49b` 交付；AC-081/083 的真实托管 completion 仍为 hosted-unverified。后续用户已授权 Phase 18.1，当前动作以本文件顶部和 Phase 18.1 章节为准；仍不 push、发布、部署或再次调用真实 provider。
...
- 按稳定 operation/attempt/chunk identity 先持久化再继续拉取 provider；正常顺序固定为 `model.request.started → model.output.delta* → model.output.completed → model.usage.updated → run terminal`，completed 携带最终长度/checksum 或 artifact reference。completed intent、最终 usage result、shared-budget settlement 与未使用 delta 槽释放必须在同一 UoW 耐久提交，提交后才按 completed → usage 公开，失败恢复不得重放 provider。
...
**冻结文件所有权与 producer**：
- 生产协议/可信入口：`packages/agent-harness/src/agent_harness/models/providers.py`、`router.py`、`_router_current.py`、`_router_snapshot.py`、`invocation.py`、新建 `_invocation_streaming.py`、`_streaming_contracts.py`、`_streaming_consumption.py`、`_streaming_events.py`、`_streaming_settlement.py`、`_settlement_contracts.py` 与 `streaming.py`、`_invocation_evidence.py`、`_invocation_settlement.py`、`_settlement_publication.py`、`_settlement_validation.py`、`_settlement_evidence_validation.py`、`usage_events.py`、`models/__init__.py`、`runtime/executor.py`。`_invocation_streaming.py` 只负责策略与纵向协调，四个 `_streaming_*` 内部模块分别承载显式协作者合同、消费/跨块安全、事件持久化/发布和中断结算；`_settlement_contracts.py` 只增加封闭 provider/runtime failure domain，不进入公开 artifact。当前配置规划、冻结快照恢复、结算与重放 validator 必须逐值支持精确 `text_stream` route evidence 和封闭 cancelled outcome，并保持 `text_completion` 回归不变。成功 stream 复用 publication 私有 UoW seam 原子持久化 completed/usage/budget/尾部释放；生产 Agent executor 只能经 `build_execution_context()` 取得 `BoundModelInvocationService.stream/stream_approved`，业务输入不能提供 `usage_call_id`。
- adapter/config/composition：`packages/agent-harness/src/agent_harness/adapters/models/{pydantic_ai.py,_pydantic_ai_streaming.py,_pydantic_ai_client.py,fake.py}`、`config/{schemas.py,settings.py}`、`runtime/{services.py,_orchestrator_base.py}`、`templates/service-app/app/runtime.py`；stream 私有协作者不增加公共导出。
- event/storage：`packages/agent-harness/src/agent_harness/events/{capacity.py,bus.py,types.py,local_capacity.py}`、`packages/agent-harness/src/agent_harness/events/sinks/{postgresql.py,_postgresql_streaming.py}`、`storage/{event_capacity_repositories.py,evidence_repositories.py,usage_evidence_repositories.py,usage_attempt_review_repository.py,_shared_budget_replay_repository.py}` 与新建 `storage/stream_evidence_repositories.py`；local/PostgreSQL sink 必须在写事件和递减容量前原子核对 stream 完整绑定，partial/unknown attempt 必须与共享预算在同一 UoW 进入 needs-review。两个新私有协作者只复用现有 connection/session，不改变 schema、UoW 属性或公共 API。
- 新增聚焦合同：`tests/contracts/model_streaming_sdk_event_test_helpers.py`、`tests/contracts/controlled_model_streaming_context_typecheck.py`、`tests/contracts/test_controlled_model_streaming_provider_contracts.py`、`tests/contracts/test_controlled_model_streaming_routing_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_success_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_interruption_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_cancellation_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_started_cancellation_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_composition_close_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_replay_contracts.py`、`tests/contracts/test_controlled_model_streaming_runtime_guardrail_contracts.py`、`tests/contracts/test_controlled_model_streaming_security_contracts.py`、`tests/contracts/test_controlled_model_streaming_capacity_contracts.py`、`tests/contracts/test_controlled_model_streaming_capacity_config_contracts.py`、`tests/contracts/test_controlled_model_streaming_recovery_contracts.py`、`tests/contracts/test_controlled_model_streaming_postgresql_contracts.py`、`tests/contracts/test_controlled_model_streaming_transport_contracts.py`、`tests/contracts/test_controlled_model_streaming_live_smoke_contracts.py`、`tests/contracts/test_controlled_model_streaming_approval_contracts.py` 与 `tests/integration/test_controlled_model_streaming_live_smoke.py`；同时扩展 `controlled_real_model_policy_approval_test_support.py` 的真实容量感知 composition。拆分只按配置/identity、取消/partial、started 后 telemetry 取消与活动 context composition close 窗口组织，原节点行为语义不变。静态类型夹具由仓库 Pyright 分析，不直接导入 vendor SDK；routing contract 必须覆盖当前路由、快照恢复、结算/重放 route evidence 的成功与篡改负路径。全量验证另精确修正 `model_usage_recovery_test_support.py`、approval-outbox recovery 与 local crash recovery 三个旧夹具，生产 validator 不放宽。既有 transport 回归精确为 `tests/contracts/test_sse_http_openapi_contracts.py`、`tests/contracts/test_cli_event_stream_contracts.py`、`tests/contracts/test_sse_event_reader_local_contracts.py`、`tests/contracts/test_sse_event_reader_postgresql_contracts.py`、`tests/integration/test_sse_http_streaming_contracts.py`。
- live/CI/验收：新建 `scripts/live_model_stream_contract.py`、`scripts/live_model_stream_probe.py`、`scripts/live_model_stream_execution.py` 与薄 CLI `scripts/smoke_live_model_stream.py`，修改 `Makefile`、`.github/workflows/ci.yml`、`.gitlab-ci.yml`、`compliance/ci-jobs.toml`、`scripts/ci_evidence.py`、`tests/contracts/test_ci_pipeline_contracts.py`、`docs/acceptance-matrix.md`，建立独立 `smoke-live-model-stream` / `ci-smoke-live-model-stream` 与安全 artifact；双 CI `acceptance-validate` 显式依赖并下载该 artifact，默认 CI 零网络并 skip live job。
- 契约/计划：`Product-Spec.md`、`Product-Spec-CHANGELOG.md`、`API-Contract.md`、`DEV-PLAN.md`、`openspec/changes/archive/2026-07-30-controlled-model-streaming/**`、`docs/plans/architecture-evolution-plan.md`、`docs/plans/architecture-evolution-change-matrix.md`。`Product-Spec-CHANGELOG.md` 只同步 AC-087 public/internal reader 可见性，不把 Phase 18.2 规划写成当前实现；实现期发现的清单外 owner 已在最终冻结前补回 proposal/design/tasks/matrix/DEV，并由最终 Reviewer 1 重新审查完整内容。
...
- 默认 quality/test/eval/smoke-local 不触网；live streaming smoke 继承 Phase 18 的四前置状态机：缺本会话授权、opt-in、隔离凭据或受信 endpoint 任一项时保持零调用并记录 hosted-unverified/CI skipped；本地 terminal/capacity/shared-budget/publication 失败记录 failed/contract_failure 与 CI fail；只有前置完整且已授权后的网络、配额或 provider 故障才记录 external-blocked。
...
## Phase 18.2: 受控跨 deployment/provider fallback（`controlled-multi-provider-failover`）

**目标**：在 Phase 18 的 typed deployment/immutable route/budget/evidence 与 Phase 18.1 的 stream/cancel/partial-result 边界上，把同 provider model fallback 扩展为多个真实 deployment/provider 的有序 route chain；只在当前候选全部实际 attempts各自耐久证明 `client_not_started|trusted_business_not_started`且无悬空项时切换，unknown、无完整受信证明的 response、已观察 usage/text或已提交 delta时停止。

**交付内容**：
- 创建聚焦 OpenSpec change `controlled-multi-provider-failover`；先更新 API/config/evidence contract，冻结 route-ref schema、chain identity、候选顺序、逐 attempt 两类可信 not-started proof、其他 started/response/timeout 围栏、预算 transfer、恢复和稳定 exhausted error，经 strict 与 fresh 契约 Stage 1/2 PASS 后才实现。
- 将 Agent model policy 从单一 deployment 内的 model fallback扩展为有序 `(deployment_id, model_id)` route refs；chain mode既有 provider/deployment/default/allowed/fallback字段只保留请求缩权前原始 Agent `fallback_routes[0]`的确定性兼容投影并前置校验，不授权后继，也不随 request删除首 route而改写。每个候选分别执行 deployment∩Agent∩request缩权，request只能选择/删减，不得新增或重排 provider、endpoint、credential或 model；Agent `[A,B]`、request `[B]`的 golden identity仍保存 A投影与完整 `[A,B]`，candidates仅为 B。
- Router 在任何 client/network 副作用前冻结完整 chain，以及每个候选自己的 provider kind、endpoint/credential identity、catalog/capability、deadline/retry/Bulkhead 和 token/cost 上界；reload 只影响新 root run。
- Composition root 通过窄 provider factory/adapter registry 为各 deployment 提供 lazy client lease；同一 provider kind 的不同 vendor endpoint/credential 作为不同 deployment，候选之间禁止共享 secret、client、permit、catalog 或 SDK 类型。
- 静态 hard eligibility 允许零调用跳过候选。Legacy单route逐字保持Phase 18的`reservation → permit → client → durable side_effect_started → send`；显式chain唯一顺序为`candidate reservation → durable attempt started identity → Bulkhead permit → candidate-isolated client/prepare → send/iterate`。每次首次调用或同 route retry都按chain顺序先创建全局连续、不可覆盖的attempt started identity；permit/client/prepare在send前失败时以`client_not_started` proof原子关闭同一lifecycle，崩溃或commit-ack未知则保留reservation/needs-review。每个已发送attempt还可凭endpoint-bound classifier、显式`cross_provider_failover_http_statuses`与零生成/计量事实共同证明的`trusted_business_not_started`收敛。只有当前候选全部lifecycle records均已证明not-started且与proof一一匹配时才可跨provider。started mark后send前、send后proof/settlement前或commit-ack未知均不重发、不创建下一attempt或后继provider。403默认不启用，同route retry先于跨provider；无证明的timeout/response/unknown/response identity/usage/text/delta立即停止，fake不作为隐式尾部。
- 每个候选使用自己的冻结 catalog/envelope/price/attempt policy建立 reservation；legacy identity保持`budget-operation-v1`，显式chain强制`budget-operation-v2`并以ordinal 1兼容投影、完整chain digest/count稳定绑定，恢复不因余额选择改写identity。静态不合格与 soft-threshold/current-balance预算不可用分别以零 attempt/proof/reservation的 `static_ineligible`、`budget_ineligible/soft_budget|balance`耐久跳过，普通中间skip不单独追加transition。上一候选全部实际 attempts先以完整 proof list耐久收敛为 actual zero；同一 owner lock/CAS UoW跨过中间不可用候选把 reservation直接转移到首个 eligible后继，或在无后继时释放并提交 exhausted terminal，首次 durable决定不随后来余额漂移。Route chain在首次可信入口、approval record前冻结 usage call id与 operation identity；waiting state、approval record/grant、shared-budget activation、settlement/outbox与 stream group复用原 claim，resume从私有 checkpoint重算，禁止 `approved:<approval_id>` rekey或第二 claim。Transition table逐值封闭；成功activation只写同ordinal`approved/approval_granted`、released=`0/null`、reserved=目标冻结bound并直接waiting→active，禁止重复activated，commit-ack只重放同一sequence/数组。获批目标 current balance不足时保留双binding与既有waiting coordination、禁止本ordinal reservation activation，并让该ordinal成为跨普通skip不变的零impact source anchor；后继独立Policy/HITL的allow/waiting/deny/exhausted各有唯一tuple。初始前导skip不成为source，allow/waiting从null开始、deny零transition。Unknown、悬空或缺失/覆盖/重排 proof保留原 reservation与 needs-review，恢复只续接同一 chain/ordinal/settlement和下一尚未开始的全局 attempt。
- 扩展 provider-neutral evidence：route-chain identity、candidate ordinal、去敏 failover reason、实际 deployment/provider/model、不可变attempt identity及`started|not_started_proven|unknown|settled` lifecycle、逐 attempt request/response/status/观察位/not-started proof/usage/cost/latency与 reservation transition；lifecycle、candidate有序proof records和公开attempts逐值交叉验证，不保存完整URL、header、secret、raw response或SDK exception。
- 补充route-chain取消的字段级终态：只有provider-neutral stream关闭结果证明`stopped + complete usage`且完整durable group无delta intent/发布不确定性时，才以actual结算`cancelled/invocation_cancelled`，candidate/lifecycle/claim进入result_committed、selected为空且不发布completed；prepared不构成provider调用事实，close失败、partial/null/unknown/not-started仍进入needs-review且绝不fallback，稳定取消或unknown分类形成后的本地cleanup异常不得覆盖既有稳定错误。
- 建立非流式与流式公共 seam 红灯、SQLite/PostgreSQL crash/replay/预算竞争合同、两 provider doubles、default fake/local 离线回归，以及仅在用户另行授权时执行的双真实 deployment/provider opt-in smoke；live artifact validator固定两条`max_attempts=1` route、ordinal `[1,2]`、selected→唯一completed、candidate/top-level attempt/proof count、usage token/cost组合和durable evidence逐值绑定的正反合同。

**非目标**：
- 动态 provider discovery、健康权重、流量分配、自动成本套利、热重载控制面、多区域运维和跨 provider 账单对账；
- structured output/tool-call 的跨 provider 语义修复；Phase 19/20 只能在各自契约中另行扩展；
- 观察或提交首个 streaming delta 后切换 provider，或由 SSE/CLI reader/reconnect 触发 provider 调用；
- 把 fake 作为真实 provider 故障后的静默成功路径。

**关键文件候选**：
- `packages/agent-harness/src/agent_harness/config/{schemas.py,settings.py,model_catalog.py,model_endpoints.py}`、Agent registry descriptor/loader 与 profile/template config - typed route refs、候选交集和安全配置；
- `packages/agent-harness/src/agent_harness/models/{__init__.py,providers.py,router.py,_router_contracts.py,_router_current.py,_router_current_chain.py,_router_snapshot.py,_router_snapshot_chain.py,invocation.py,_invocation_execution.py,_invocation_planning.py,_invocation_streaming.py,_invocation_approval_identity.py,_invocation_chain.py,_invocation_chain_base.py,_invocation_chain_routing.py,_invocation_chain_approval.py,_invocation_chain_completion.py,_invocation_chain_stream.py,_invocation_chain_evidence.py,_invocation_chain_settlement.py,_route_chain_state.py,_route_chain_state_initial.py,_route_chain_state_approval.py,_route_chain_state_attempts.py,_invocation_evidence.py,_invocation_settlement.py,_settlement_contracts.py,_settlement_validation.py,_settlement_evidence_validation.py,_settlement_evidence_models.py,_settlement_chain_evidence_validation.py,_settlement_publication.py,_streaming_consumption.py,_streaming_events.py,_streaming_settlement.py}` - frozen chain、公共 route-chain DTO 导出、durable approval identity恢复、failover protocol、首 delta 耐久围栏、attempt/evidence、settlement publication 与恢复；内部职责拆分不改变公共 façade、DTO、事务边界或顺序语义；
- `packages/agent-harness/src/agent_harness/adapters/models/{pydantic_ai.py,_pydantic_ai_client.py,_pydantic_ai_streaming.py,fake.py}`、`runtime/{services.py,shared_budget.py,_shared_budget_identity.py,_shared_budget_snapshot.py,_shared_budget_recovery.py}` - provider factory/lifecycle、全链 claim 高水位与逐候选状态 identity、预算 transfer 与 replay fencing；
- bound `complete_approved()`/`stream_approved()`、`packages/agent-harness/src/agent_harness/runtime/{continuation.py,_run_continuation.py}`、`approvals/{service.py,_continuation.py}`、`storage/{access_repositories.py,approval_records.py,approval_recovery_repositories.py,service_approval_repositories.py}` - approval前 usage/operation identity冻结、私有 checkpoint重算、request/grant两阶段 binding、approval lease独立 UoW fencing与 shared-budget activation durable handoff；不得 rekey、新建第二 claim、公开 lease私有字段或伪装跨 repository原子消费；
- `packages/agent-harness/src/agent_harness/storage/{shared_budget.py,shared_budget_models.py,model_route_chain_state.py,_model_route_chain_recovery.py,_shared_budget_route_chain_repository.py,_shared_budget_route_chain_validation.py,_shared_budget_repository_records.py,_shared_budget_direct_repository.py,_shared_budget_allocation_repository.py,_shared_budget_replay_repository.py,_shared_budget_lifecycle_repository.py,shared_budget_repositories.py,usage_evidence_repositories.py,stream_evidence_repositories.py,usage_attempt_review_repository.py}` 与 SQLite/PostgreSQL migration catalog/runner - durable row、ORM→领域记录映射、settlement/snapshot/recovery、stream 全局 attempt validator 与 unknown/needs-review 持久化；另含共享失败域producer `scripts/smoke_live_model.py`、`scripts/live_model_failover_evidence.py`、`scripts/live_model_failover_contract.py`、`scripts/smoke_live_model_failover.py`、CI producer、`API-Contract.md`、验收矩阵、双语维护文档及 Phase 18.2 聚焦 contracts/integration tests。职责拆分后的测试 owner 包含 `controlled_multi_provider_failover_settings_test_support.py`、`controlled_multi_provider_failover_test_support.py`、`test_controlled_multi_provider_failover_approval_checkpoint_contracts.py`、`test_controlled_multi_provider_failover_invocation_contracts.py`、`test_controlled_multi_provider_failover_policy_budget_contracts.py`、`test_controlled_multi_provider_failover_recovery_contracts.py`、`test_controlled_multi_provider_failover_stream_cancellation_contracts.py`、`test_controlled_multi_provider_failover_postgresql_candidate_state_contracts.py`、`test_controlled_multi_provider_failover_live_smoke_orchestration_contracts.py`、`test_shared_parent_budget_route_chain_repository_contracts.py`、`test_shared_parent_budget_route_chain_allocation_contracts.py`、`test_shared_parent_budget_route_chain_candidate_state_contracts.py`、`test_shared_parent_budget_route_chain_transition_contracts.py` 与共享executor相邻`test_controlled_real_model_offline_contracts.py`，既有验收节点名称保持不变。

**验收标准**：
- AC-090 至 AC-095 全部先从公共 route/invocation/composition/recovery seam 建立可复现红灯，再实现；不得用私有 helper 或同 provider model fallback 代替跨 deployment/provider 行为。
- 至少两个不同真实 deployment doubles 证明凭据/endpoint/client/Bulkhead 完全隔离、request只缩权、候选顺序冻结、两类 safe not-started各切换一次，并以同候选两次受信 retry和三候选双顺序证明调用级/候选级高水位不覆盖逐 attempt资格；无受信证明的 started/response/timeout/unknown/partial/delta为零后续 provider调用。
- 逐候选不同价格、token envelope和 attempt policy在 local/SQLite与真实 PostgreSQL中逐值验证调用级高水位、逐候选聚合 side-effect state、attempt-start identity、不可覆盖 proof append、attempt evidence交叉验证、A→C预算 skip直达、无后继原子释放/terminal、approved canonical transition、非法from/to/reason/bound与重复activated拒绝、approved目标 balance skip零approved/activated、零impact source anchor下后继allow/waiting/deny/exhausted四分支、初始前导skip后null-source waiting/deny、reservation transfer、actual settlement、同候选/三候选 crash replay与 terminal fencing；特别覆盖上一proof已提交后下一started mark提交、send前崩溃和send后proof/settlement前崩溃，两个数据库都不得重发、创建下一attempt或切provider，也不按恢复时新余额重选。
- Phase 18.1 streaming 回归证明首 delta 前只允许 safe not-started failover，首 delta 后 cancellation/deadline/reader reconnect/recovery 均不切 provider。
- Completion/streaming公共bound回归须覆盖prepared后取消零观察、cleanup失败不覆盖稳定错误、stream close的stopped complete/partial/null/unknown矩阵；可信actual取消必须逐值验证cancelled state、actual charge、空selected/current reservation、零completed/后继调用，其他分支验证needs-review与如实`provider_called`。
- 默认 quality/test/eval/smoke-local 不读取真实 credential 且零网络；双真实 deployment/provider smoke 只有用户另行授权、opt-in、两个不同 `deployment_id`、两个隔离 `credential_ref` 和两个不同受信 endpoint 齐全时运行，两个 deployment 的 `provider_kind` 可以相同，并用同 kind 双 deployment 正合同锁定；否则准确记录 hosted-unverified，只有前置完整后的外部故障才记录 external-blocked。
- `make quality`、聚焦 contract/integration、`make test`、`make eval`、`make smoke-local`、触及 service composition 时的 `make smoke-service`、build/license/strict/diff checks 与 fresh 实现 `1+2` review 全部按 change 冻结范围收口。

**依赖与并行**：强依赖 Phase 18.1 验证、fresh review、同步并归档。Config、route chain、provider factory、budget settlement、recovery、tests/docs 共同决定“只在可证明未开始时跨 provider”的安全不变量，单一 worktree、单一 owner 串行；只读厂商能力调查与威胁建模可并行。Phase 19 必须从 Phase 18.2 归档 HEAD 开始。
...
**交付内容**：
- 三个串行、关联的 OpenSpec change 依次固定：20A provider-neutral tool intent 与零执行解析；20B `模型决策 → ToolRegistry → Policy/HITL → 工具结果 → ContextAssembler → 模型续跑`；20C loop/turn/tool-call identity、checkpoint/resume、exact replay、unknown 与 terminal fencing。三个 change 共享 DTO、CanonicalEvent、预算、恢复验收和文件所有权，分别 strict/review 后仍须联合审查。
- 20A 公开 exact `ToolCatalogSelection`，由 bound tool-intent seam 区分缺省完整 catalog、显式空 catalog 与唯一保序子集；它不扩展 `ModelRequest`，非法选择在模型副作用前拒绝。
...
**验收标准**：REQ-029 / AC-104～AC-111 逐项闭合。未知/越权工具零副作用；危险动作只能等待审批；恢复不重复调用；turn/token/cost/tool-output/wall-clock 与 shared budget 生效；所有 tool output 以 untrusted source 进入 ContextAssembler。默认 fake/tool stub 零网络；真实模型与真实外部工具的联合 smoke 只有另行授权后才可执行，不能作为离线交付前置。
...
| REQ-004 配置系统 | Phase 2, Phase 12, Phase 13.6, Phase 13.8A（fingerprint typed secret）, Phase 18（model deployment/credential/endpoint）, Phase 18.1（stream capability 与有界策略）, Phase 18.2（有序 route refs 与逐 deployment 隔离） |
...
| REQ-012 模型、预算、上下文组装与 embedding | Phase 2, Phase 4, Phase 6, Phase 13.6A, Phase 13.7, Phase 13.8A, Phase 18, Phase 18.1, Phase 18.2, Phase 19, Phase 20 |
...
| REQ-029 受控模型工具循环与耐久恢复 | Phase 20A、20B、20C；provider-neutral tool intent、Registry/Policy/HITL 执行链、untrusted result 回注、全局边界、checkpoint/resume、exact replay 与 unknown/terminal fencing |
...
| 真实模型若绕过 typed deployment、冻结 route、预算、策略、审计或证据链，会把“能请求 SDK”误当成“已受控运行”。 | config、router、composition、budget、audit、service template。 | Phase 18 | 实现、严格 `1+2` review、同步、归档与本地提交已完成；外部成功未验证 | `controlled-real-model-runtime` 已按 red-first 建立 lazy 真实 provider composition 与完整治理路径；fresh Reviewer 1/2/3 在同一冻结实现身份上均 Stage 1/2 PASS，change 已由 `ff0c49b` 交付。两次授权 MiMo 入口仍未建立真实成功，第二次按 side-effect unknown 处理且不再重试。 |
...
| SSE 若复用 JSON route 或引入第二套事件状态，会造成 resume 漂移、visibility 泄漏和代理缓冲。 | Access、CanonicalEvent、OpenAPI、service deployment。 | Phase 13.9；Phase 18.1 复用 | provider producer 已完成审查、同步并归档 | RUN-006 与 CLI-EVT-001 复用统一授权 EventSink reader，Last-Event-ID 为唯一 SSE 续读输入，握手前/后错误分离，并已验证 transport 背压、真实 service resume 与已有事件首 frame；Phase 18.1 producer 只写 committed events，不增加 provider cursor，真实 provider streaming 仍为 hosted-unverified。 |
