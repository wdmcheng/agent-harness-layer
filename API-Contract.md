...
- `route`：除下述显式条件可空字段外，ID/ref/version/origin/capability/schema 字段为非空 string；`endpoint_policy_digest` 与 `model_catalog_digest` 均为 64 位小写十六进制。真实 route 的 `credential_ref`、`model_catalog_ref/version/digest` 与 `request_shape_ref/version` 必填，fake 的真实网络字段可为 null。真实 route 的模型目录 identity 必须逐字命中 `ModelSettings.model_catalogs` 的同一 canonical entry，`request_shape_ref/version` 固定为 `single-user-text-no-tools`/`v1`；deployment 的 allowed model 只保存 exact catalog ref/version，不能另行声明权威 envelope/price。`completion_classifier_ref/version` 必须同时为 null 或同时为非空 string；非空时 exact identity 仅为 `trusted_response_header_not_started`/`v1`，且必须由当前 endpoint policy/version 授权并计入 digest。真实 route 的 `input_bound_strategy_ref` 固定为 `utf8-bytes-plus-envelope`、`input_bound_strategy_version` 固定为 `v1`，且必须与同一模型目录 entry 匹配；`input_envelope_token_bound`、`prompt_utf8_bytes`、`trusted_input_token_bound`、`output_token_cap`、其他 token bounds、`max_attempts` 与 timeout 均为非 bool integer，其中 envelope/prompt/bounds `>=0`、output cap/attempt/timeout `>=1`。Deployment 静态 `max_per_attempt_token_bound` 必须等于 `max_prompt_utf8_bytes+catalog.input_envelope_token_bound+max_output_tokens`；cost enabled 时两项价格为有限非负 Decimal，`price_source_ref/version` 均为非空 string，静态 `max_per_attempt_cost_bound` 必须等于 `(max_prompt_utf8_bytes+catalog.input_envelope_token_bound)*input_price+max_output_tokens*output_price`；cost disabled 时两项价格、`price_source_ref/version`、静态 `max_per_attempt_cost_bound`、动态 `per_attempt_cost_bound` 与 `reserved_cost_bound` 均为 null。具体 route 的 `trusted_input_token_bound=prompt_utf8_bytes+catalog.input_envelope_token_bound`、动态 `per_attempt_token_bound=trusted_input_token_bound+output_token_cap`、`reserved_token_bound=per_attempt_token_bound*max_attempts`；cost enabled 时动态 `per_attempt_cost_bound=trusted_input_token_bound*input_price+output_token_cap*output_price`、`reserved_cost_bound=per_attempt_cost_bound*max_attempts`。所有公式使用 checked arithmetic，动态值不得超过静态 ceiling；catalog ref/version/provider/model/request-shape/strategy/price/digest 未知或不匹配、deployment 复制值低报或高报、任一公式不一致、prompt/output 超 deployment cap、价格或其来源 identity 缺失、非有限或溢出均在 reservation/client/network 前拒绝。Route、`budget-tree-v2` snapshot、operation identity 和 evidence 冻结同一目录 identity 与解析值，恢复不读取 current catalog 补齐或改价。Adapter 必须把 `output_token_cap` 逐字传为 Pydantic AI `ModelSettings.max_tokens`。`retry_policy` exact shape 为 `{retryable_http_statuses: integer[], max_attempts: integer, max_wait_ms: integer, backoff_initial_ms: integer, backoff_max_ms: integer}`；状态码去重、升序，且每项只能是 `429` 或 `500..599`，其余数值非 bool、非负，`max_attempts>=1`。未绑定 classifier 时 `retryable_http_statuses` 必须为空；绑定时只有 transport 从当前 endpoint 的 exact 单值 `X-Agent-Harness-Completion-State: not-started` 得到可信 false 且不存在 response id/usage/partial result，才可按该数组 retry。缺失、重复、多值、畸形、不可信来源或 body/exception 伪造均为 unknown/no-retry。`bulkhead_policy` exact shape 为 `{scope:"process_deployment", max_in_flight: integer, queue_timeout_ms: integer}`，两个整数均非 bool 且 `>=1`。
