...
| 字段 | 约束 |
|---|---|
| 状态 | 契约已冻结，当前实现与验收待完成；仅覆盖单 provider kind 的非流式 text completion，不包含 streaming、structured output、tool calling、多 provider 路由或生产流量切换。 |
...
| DTO / route | `ModelRequest` 不声明 endpoint/credential override 且 `extra=forbid`；未声明字段返回 `model.request_invalid`。Router 只对 deployment/model/provider assertion/capability 做缩权；`provider_kind`、Agent/request provider assertion、route/evidence `provider` 和 adapter `provider_id` 在 `MOD-002` execution seam 使用同一 `fake|openai-compatible` identity，`pydantic-ai` 不属于公共 identity。失败分别使用 `model.route_not_allowed` 或 `model.capability_unsupported`。 |
