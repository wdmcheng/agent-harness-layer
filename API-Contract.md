...
该目录是合法 `CanonicalEvent` 类型词汇，不是 producer 完成清单。既有一次性模型调用只形成 request-started 与最终 usage evidence；`controlled-model-streaming` 按 5.9.1 生产 `model.output.delta` / `model.output.completed`，其 provider → router → invocation → durable event 生产链已经实现。RUN-006 / CLI-EVT-001 仍只传输已经持久化的事件，不形成第二个 provider producer。
...
- `controlled-model-streaming` 的 producer 契约固定在 5.9.1；后续 change 不得改变受信 stream operation kind、最大 delta 数、分片/合并策略、稳定 event id、可见性、安全、顺序或 unknown 规则后再反向修改契约。SDK token 数量或 HTTP/Agent 自报值不得作为容量上限；不能在无界 provider stream 已开始后才发现 CanonicalEvent 容量不足。
...
#### 5.9.1 MOD-004 受控真实模型增量文本 producer

本节是 `controlled-model-streaming` 的冻结 API/event contract，生产链已经实现。它只增加 provider-neutral 普通文本 producer，不新增 HTTP/WebSocket route，不透传 SDK event、provider cursor、reasoning/tool/structured delta，也不改变 MOD-002 一次性 `complete` seam。默认 provider 仍为 fake；真实流式验证必须同时取得既有真实 provider opt-in 与独立 stream opt-in，默认 CI 不发起网络请求。

| 契约面 | 冻结语义 |
|---|---|
| 业务入口 / capability / provider seam | 生产 Agent executor 只可从 `build_execution_context()` 取得绑定到可信 identity/tenant/run/agent/request/trace 的 `BoundModelInvocationService`。该 façade 增加异步 `stream(request, operation_key=...) -> ModelResponse` 与 `stream_approved(request, operation_key=..., grant=...) -> ModelResponse`，并在内部调用底层 `ModelInvocationService.stream`；业务输入不得提供 `usage_call_id` 或未绑定 context。两入口只在 durable completed/usage 闭合后返回最终响应，delta 只进入 CanonicalEvent，不向 executor 暴露第二个 iterator。普通入口以可信 context + 语义 `operation_key` 生成稳定 call identity；审批入口复用既有 grant 全绑定、单次 lease 与 current hard-gate 重检，identity 槽位固定为 `approved:{grant.approval_id}`，不得靠更换 `operation_key` 扩成第二次调用。普通入口命中 `require_approval`、或审批 grant 缺失/过期/已消费/不匹配时，必须在两笔容量预留、started、client send 与 provider 迭代前按既有稳定错误停止；SSE/CLI 仍只读取 committed events，不成为 provider 调用入口。route 仅以 `capability=text_stream` 声明普通文本流能力；provider 必须实现独立 `ModelStreamingProvider` / `PreparedModelStreamCall`，prepare 阶段不得开始 SDK 迭代或网络副作用。不支持路径在 reservation、started、client send 前返回既有 `model.capability_unsupported`，不得把 `complete` 结果切片伪装成流。公开 seam 只包含追加式文本增量、provider-neutral 最终 `ModelResponse` 与 `ModelStreamCloseResult`；SDK 类型、cursor、tool/reasoning/structured 事件保持 adapter 私有。`ModelStreamCloseResult` exact fields 为 `state=not_started|stopped|unknown` 和 nullable `usage`。`usage` 若存在必须是 `ModelStreamUsage` exact shape：`finality=partial|complete`、nullable 非 bool 非负 integer `input_tokens|output_tokens`、nullable 有限非负 `cost_usd`、`cost_status=reported|estimated|unavailable`、非 bool 非负 integer `latency_ms`；`cost_usd=null` 当且仅当 `cost_status=unavailable`。`not_started` 强制 usage null；`stopped` 允许 null/partial/complete；`unknown` 只允许 null/partial，禁止把 complete 冒充已停止证明。adapter 只能从 SDK usage 转换为该 DTO，invocation 不得读取 SDK object。 |
| 容量与 outbox | 版本化 `operation_kind=model_stream` 固定预留 `65`：`64` 个 delta 占位 + `1` 个 completed 占位；既有 `operation_kind=model_usage` 仍固定预留 `2`：started + final usage。两笔预留、65 个 `run_evidence_outbox` 占位和关联 identity 必须在同一 UoW 原子提交，且早于 provider 第一次可能副作用；任一失败为零 provider 调用。每个 stream 占位 `reserved_event_count=1`、共享稳定 group、`sequence_in_group=1..65`，初始 `started`；使用时依次经历 `result_persisted -> published`。成功结束时未使用 delta 占位改为 `cancelled`，并只从 outstanding 释放同数量未消费容量，不推进 high-water；local `events/local_capacity.py` 与 PostgreSQL `events/sinks/postgresql.py` 必须在写真实事件并递减 outstanding 的同一原子边界逐值核对 event id、group、sequence、event type 与 payload identity，再消费一个 outstanding 并推进一个 high-water。unknown 保留全部未决占位和两笔 reservation，阻止 terminal。 |
| 稳定 identity | `usage_call_id` 是由 tenant/run/request/agent/trace 与语义调用槽位生成的 64 位小写 SHA-256，已包含 tenant 关联，不含 prompt/secret。stream group id 固定为 `model-stream:{usage_call_id}`，长度 `77`。delta event id 固定为 `model-stream:{usage_call_id}:d:{chunk_ordinal}`，`chunk_ordinal` 使用无前导零十进制 `1..64`，最大长度 `82`；completed event id 固定为 `model-stream:{usage_call_id}:c`，长度 `79`。配套 usage started/final 使用 `stream-usage-v1` identity：`usage-stream:{usage_call_id}:s` 与 `usage-stream:{usage_call_id}:f`，长度均为 `79`；usage outbox 的 `event_id` 绑定 final identity。上述值均不超过现有 PostgreSQL `VARCHAR(128)`，不得重新拼接原始 `tenant_id` 或 provider identity。新 `text_stream` 的 durable started `ModelUsageEvidence.decision.usage_event_identity` exact shape 固定为 `{"ref":"stream-usage","version":"v1"}`；其他值或字段缺失对新 stream 均 fail closed。历史/非流式 durable row 缺少该对象时继续使用既有 `usage:{tenant_id}:{usage_call_id}:started|final`，恢复不得重命名或重键历史 event。最大长度 tenant 与 ordinal 64 必须在 SQLite、真实 PostgreSQL 逐值验证 stream group、delta、completed、stream usage started/final 五种 identity。 |
| delta envelope | `event_type=model.output.delta`、`event_version=1.0`、`record_scope=run`、`visibility=public`、`terminal=false`。payload exact shape 为 `{"correlation":{"usage_call_id":"<64-hex>"},"attempt":1,"chunk_ordinal":<1..64>,"text":"<non-empty>"}`；未知字段拒绝。每条 `text` 最多 `4096` UTF-8 bytes，且完整 CanonicalEvent 仍受 65536-byte envelope 上限。运行时目标分片配置范围 `1..4096`、默认 `1024`，只可缩小目标，不改变 64/65 硬合同。provider fragment 边界不属于公共身份；producer 在 Unicode 字符边界合并/切分，不发布空 delta，不丢失、重排或静默截断。 |
| completed envelope | `event_type=model.output.completed`、`event_version=1.0`、`record_scope=run`、`visibility=public`、`terminal=false`。payload exact shape 为 `{"correlation":{"usage_call_id":"<64-hex>"},"attempt":1,"chunk_count":<0..64>,"text_utf8_bytes":<non-negative-int>,"text_sha256":"<64-hex>"}`；未知字段拒绝。按 ordinal 拼接的安全 delta 文本必须与同一安全处理后的最终 `ModelResponse.output_text` 逐 UTF-8 byte 相等，byte count 与小写 SHA-256 同时匹配才可发布；空输出允许 `chunk_count=0`。不一致时不得发布 completed、final usage 或 run terminal。 |
| 跨 chunk 安全 | 任何 delta 在持久化前必须经过有状态 incremental text guard。guard 保留可能形成既有凭证触发词的最长后缀，以及从已识别触发词到确定终止符的完整敏感候选；只有证明不属于候选的前缀才可交给分片器。`sk-`、authorization、cookie、api_key、password、secret、token 等现有规则跨任意 provider fragment 边界仍按 `redact_secrets()` 的自由文本语义整体替换为 `[REDACTED]`；其中 `api_key`、`password`、`secret`、`token` 不要求左侧单词边界，内嵌于 `OPENAI_API_KEY`、`db_password`、`client_secret`、`access_token` 等配置名时也必须遮蔽匹配的键值片段。敏感候选硬上限配置范围 `128..4096` UTF-8 bytes、默认 `512`；溢出时不得释放候选任何字节，关闭 stream 并按停止证明分类。需要完整结果的 output guardrail 禁止 speculative delta，使用最多 `64*4096` UTF-8 bytes 的有界 collector。`ModelStreamDelta` 在进入 adapter/invocation collector 前以不构造整段 UTF-8 bytes 的有界计数拒绝单个超大 fragment，并缓存合法 fragment 的字节数；adapter 只有在累计值仍不超过总上限时才保存 fragment，invocation 复用缓存值而不再次编码整段文本。collector 结束时才 join 一次，避免任意大输入先入列或高碎片输入形成二次方复制；超限时不扩容并立即关闭失败。 |
| Pydantic AI 2.5.0 adapter | adapter 使用锁定版本 `Agent.run_stream_events` async context manager；只把 `PartStartEvent(TextPart.content)` 与 `PartDeltaEvent(TextPartDelta.content_delta)` 转为追加文本，忽略 `PartEndEvent` 的重复完整 part，并隔离 tool/reasoning/structured/其他事件。唯一尾部 `AgentRunResultEvent` 提供最终 output 与 provider usage；SDK usage 在单次 adapter 生命周期内只读一次并缓存 provider-neutral 转换，accessor 抛异常或 bool、负数、非整数等非法值时，`result()` 稳定返回 `model.provider_side_effect_unknown`，`aclose()` 复用缓存返回 `state=unknown` 而不再次读取或逃逸原始异常。缺失/重复 final、非文本 final 或增量/最终不一致均不得合成成功。禁止使用会跳过 result validator 的 `stream_text(delta=True)`，也不把 context 本地清理冒充远端已停止证明。 |
| 顺序与结算 | 成功 committed seq 顺序固定为内部 `model.request.started` -> `model.output.delta` 0..64 条 -> 公共 `model.output.completed` -> 内部 `model.usage.updated` -> 唯一 run terminal。每条 stream event 先固化完整 payload 到 `result_persisted`，调用方登记该 durable chunk 后再由 EventBus 发布，最后标 `published`；sequence `n` 发布前必须锁定并观察到恰好且唯一的 `1..n-1` 前驱，缺失、重复、非连续或未 `published|cancelled` 时关闭失败，空查询不能冒充已结算。若 delta intent 已提交而本次 persist 返回确认、publish、telemetry 或 mark 任一未闭合，即使进程内 chunk 计数仍为 0、adapter 随后回报 stopped+complete，也必须扫描完整 65 槽 durable group，把该调用提升为 `needs_review`，保留 `result_persisted` delta、全部剩余占位、usage/预算/lease 与 terminal 围栏；不能用偏小的本地/已公开 chunk 计数取消该槽位或先发布中断 usage，恢复只以原 event id 补投。取得可信最终结果后，completed intent、最终 usage result、shared-budget settlement 与未使用 delta 槽释放必须在同一 UoW 提交；提交后才先发布 completed、再发布 usage。任一公开发布失败只留下可补投的两类 `result_persisted`，恢复保持相同顺序且不得重放 provider。usage 只采用 provider 最终可信值，不从 delta 数、文本长度或零值推断；completed/usage、未使用容量释放、budget settlement、provider lease 任一未闭合时 terminal 保持围栏。 |
| 取消、deadline、背压与 unknown | invocation 在进入 `prepare_stream` 前建立唯一绝对 monotonic deadline；同一个 `asyncio.timeout_at` 覆盖 prepare、SDK 消费、完整结果 guardrail、尾部分片与各 delta 的持久化/发布，prepare 后不得重新获得完整 `total_timeout_ms`。每完成一个安全 chunk 的持久化/发布后才拉取后续 SDK event；Pydantic AI 2.5.0 zero-buffer stream 提供自然背压，项目不新增 invocation queue。若当前 chunk 的 outbox persist 或 CanonicalEvent publish 不能在剩余 deadline 内完成，invocation 退出 stream context，并按 provider 停止证明分类；不得另起后台 task、扩大 deadline、缓存后续 SDK event、静默丢片或乱序。显式取消、deadline、容量/分片/安全上限、存储故障或进程退出先清理本地 stream context。若双预留事务尚未提交，`not_started` 直接随 UoW 回滚；事务已提交后不得删除或倒退任何 event/high-water。调用方取得 iterator 只表示请求首次迭代；若 deadline 在 SDK stream context 创建前耗尽，仍为可证明的 `not_started`。尤其在内部 started 已发布、SDK context 与首次 provider 迭代尚未发生的取消/deadline 窗口，必须保留 started，批量取消 65 个 stream 占位并只释放 65 outstanding，持久化并发布 `outcome=cancelled` 的 stream usage final：attempt 固定 `side_effect_state=not_started`、`provider_called=false`、token/cost 为 null/`unavailable`，budget charge 为可信零，而不是把 provider usage 伪成零；该 final 消费剩余 1 个 usage 槽位并推进 high-water。随后释放 budget reservation 与本地 permit/client lease，最后才允许 `run.cancelled`。一旦 SDK context 已创建，普通 task cancellation、本地 timeout、socket/context 关闭或本地清理只证明本地停止，不得伪造 not-started/stopped。只有 provider seam 明确证明远端无后续副作用时才可返回 `stopped` 并取消未使用 stream 占位；只有 `usage.finality=complete` 且 input/output 与当前 route 启用的 cost 维度全部可信时，才按该 DTO 生成中断 `ModelUsageEvidence`、结算预算、释放 provider lease/未消费容量并允许后续 `run.cancelled`。stopped 的 null/partial usage 与 unknown 的 null/partial usage 必须在同一 UoW 把 usage outbox、对应 shared-budget claim/allocation 和 owner ledger 提升为 `needs_review`；不得发布 final usage、释放任何剩余 stream/usage 容量、退款或写 run terminal。usage `result_json` exact shape 为 `{"started":<原 durable started evidence>,"attempt_review":<review>}`，预算 result 只保存同一 `<review>`；`review` exact fields 为 `provider_close_state`、`usage_finality`、`outcome`、`error_code`、`provider_called=true`、`latency_ms`、单元素 `attempts` 与 `budget_charge`，其中 charge 固定为 `charged_tokens=null`、`charged_cost_usd=null`、`charge_status=unknown`、`unresolved_attempts=[1]`。`unknown` attempt 的 `side_effect_state/outcome` 为 `unknown/unknown`，`stopped` partial attempt 保留真实 `cancelled|failed` outcome；公开稳定错误统一为 `model.provider_side_effect_unknown`。该状态保留已提交前缀、65 个未决 stream 占位、剩余 usage reservation、budget 与 provider lease，exact replay 只能拒绝为 needs-review，不能再次调用 provider。 |
| 恢复与 transport | recovery 只按稳定 event id 补投 `result_persisted` 的既有完整 envelope、完成本地结算或保持 unknown；不得重新构造 provider 调用、产生新 ordinal、补写猜测文本/usage。SSE `Last-Event-ID` 与 CLI `--after-seq` 只读取 committed CanonicalEvent seq；默认 public reader 显示 delta/completed 并隐藏 internal started/usage。reader 断开、取消或慢消费只结束 reader，不持有 provider stream、不修改 outbox/lease、不触发 provider 取消或重启。 |
| live 时延 evidence | opt-in smoke 输出 `model-stream-live-smoke/v1`：`status=passed|failed|hosted-unverified|external-blocked`、`provider_called:boolean`、`existing_event_first_frame_ms`、`provider_first_delta_ms`、`committed_first_delta_ms`、`client_first_delta_ms` 与 nullable `reason_code`。四个时延字段只能是非 bool、非负 integer milliseconds 或 null；smoke 在同一受控进程内协调 local runtime invocation 与事件 client，因此后三项可共用 durable started 后、首次拉取 provider event 前取得的 monotonic origin，禁止跨进程比较不同 monotonic clock。`passed` 时四项全部非 null、`provider_called=true`、`reason_code=null` 且 `provider_first_delta_ms <= committed_first_delta_ms <= client_first_delta_ms`。`existing_event_first_frame_ms` 独立从已有 committed event 的 SSE request 到首 frame 计时，仍按 AC-066 `<1000ms` 验证，不形成 provider 首 token SLA。缺本会话授权、stream opt-in、隔离凭据或受信 endpoint 任一项时必须零调用、`hosted-unverified`、后三项 null、进程 0/CI skipped，reason 分别为 `authorization_missing|stream_opt_in_missing|credential_missing|endpoint_untrusted` 中优先命中的一项；本地 setup/start-run/terminal/capacity/shared-budget/publication/policy/guardrail/probe/cleanup 或其他编排失败必须为 `failed`、进程 1/CI fail、`reason_code=contract_failure`，并按已观察 response、delta 或稳定摘要如实保留 `provider_called`。`RunOrchestrator.start_run()` 异常是独立本地失败事实，即使 executor 返回 provider-domain 错误且 probe/cleanup 成功，也不得覆盖为 `external-blocked`。in-process `ModelProviderInvocationError.failure_domain` 只能是 `provider|runtime`，executor 必须消费该封闭来源而非用成功 `ModelResponse` 是否存在或通用错误码猜测归因；该字段不进入 live artifact。只有四前置完整、已授权且不存在任一本地失败时发生网络、配额或 provider 故障，才可 `external-blocked`、进程 2/CI fail，reason 只允许 `network_unavailable|provider_rejected|quota_blocked|provider_timeout|provider_result_unknown`，已知时延必须保持上述顺序，未知值为 null。artifact 不得含 prompt、delta/final text、credential、endpoint path、header、response id 或 raw provider error；fake clock contract 在默认离线测试中逐字段验证 schema、量纲、顺序和状态映射，不能用一次总 latency 代替。 |
| schema / migration | `run_evidence_outbox` 已具备 `operation_kind`、`state`、`result_json`、`reserved_event_count`、`group_id`、`sequence_in_group`，其 identity 列与 `canonical_events.id` 均可容纳上述 stream 与 `stream-usage-v1` 定长公式；`state`/`operation_kind` 无数据库枚举约束。历史 usage identity 原样恢复，新 stream identity 版本存在 JSON evidence 中，因此本 change 不需要 schema migration。若真实 PostgreSQL 红测证明任一现有约束不足，必须先回到 change/API contract 重新审查，不得在实现中隐式改 schema。 |

本 producer 的稳定内部错误继续复用 `model.capability_unsupported`、`model.invocation_cancelled`、`model.provider_failed`、`model.provider_side_effect_unknown`、`event.sequence_exhausted`、`event.envelope_too_large` 与 storage/replay errors；分片、安全候选或最终一致性无法完成且远端状态不明时统一归入 `model.provider_side_effect_unknown`，不新增公开 HTTP status 或携带原始 provider 异常。真实 provider 的 started side effect 默认 unknown/no-retry，除非 adapter 拥有本节要求的明确停止证明。
...
| `output_schema_identity` | object | Yes | provider-neutral 输出 schema identity；exact fields 为 `schema_version="output-schema-identity-v1"`、与 `output_schema_ref` 相同的 `schema_ref`、descriptor `version` 和严格 canonical JSON Schema 的 64 位小写 SHA-256 `digest`。不得包含 Python class、module object、Pydantic AI 或 provider SDK 类型。 |
...
| `model_policy.deployment_id` | string | Yes | 稳定 canonical deployment ID；public descriptor 不含 endpoint 或 credential。 |
| `model_policy.allowed_models` | string[] | Yes | Agent 在 deployment allowlist 内进一步允许的非空去重模型集合。 |
| `model_policy.default_model` | string | Yes | 默认模型 ID。 |
| `model_policy.fallback_models` | string[] | Yes | fallback 模型 ID 列表；default/fallback 均必须属于 `allowed_models` 且不得重复。 |
...
受控真实非流式模型调用必须在 `decision` 中额外提供以下 provider-neutral 子结构；fake、embedding 与既有历史 evidence 不强制回填这些字段。`text_completion|structured_output` 沿用 no-tools 形态，`tool_intent` 按 MOD-006 使用 tool-enabled 形态及其追加身份字段：

| 子结构 | 必填字段 | 约束 |
|---|---|---|
| `route` | 所有真实非流式能力共同必填 `deployment_id`、`provider_kind`、`provider`、`model`、`endpoint_origin`、`endpoint_policy_ref`、`endpoint_policy_version`、`endpoint_policy_digest`、`model_catalog_ref`、`model_catalog_version`、`model_catalog_digest`、`request_shape_ref`、`request_shape_version`、`completion_classifier_ref`、`completion_classifier_version`、`credential_ref`、`capability`、`price_source_ref`、`price_source_version`、`input_bound_strategy_ref`、`input_bound_strategy_version`、`input_envelope_token_bound`、`input_token_price_usd`、`output_token_price_usd`、`prompt_utf8_bytes`、`trusted_input_token_bound`、`output_token_cap`、`per_attempt_token_bound`、`per_attempt_cost_bound`、`max_attempts`、`reserved_token_bound`、`reserved_cost_bound`、`connect_timeout_ms`、`read_timeout_ms`、`total_timeout_ms`、`retry_policy`、`bulkhead_policy`、`snapshot_schema_version`；`tool_intent` 还必须追加 `tool_catalog_digest`、`tool_catalog_utf8_bytes`、`max_tool_catalog_utf8_bytes`、`tool_request_identity_digest` | `provider_kind` 与 `provider` 必须逐字相等，并与 Agent/request assertion、route 及 adapter `provider_id` 使用同一稳定 `fake|openai-compatible` identity；`pydantic-ai` 只是私有实现名。公开 `endpoint_origin` 只允许 canonical origin，endpoint policy/classifier ref/version/digest 用于关联私有 snapshot 中的 canonical base URL/path 与可信 completion signal policy；公开 decision 不含完整 URL 或原始 classifier header。模型目录 ref/version/digest、request-shape、输入上界和价格必须来自同一受信 `ModelSettings.model_catalogs` entry，deployment、Agent、request 与 provider response 均不得自证或覆盖。`tool_intent` 的追加字段必须逐值投影 MOD-006 的 `tool-intent-request-identity-v1`，其他 capability MUST 不携带这些字段。`credential_ref` 只允许稳定 identity，不含 secret。边界数值必须经非 bool、非负、有限和 checked multiplication 校验；cost disabled 时两项 token price、`price_source_ref/version` 与对应 cost 必须为 null，不能解释成零价。`retry_policy` 只保存状态码、attempt/等待上限的安全摘要，`bulkhead_policy` 只保存并发与排队上限。 |
| `attempts[]` | 文本调用保持既有 `ModelAttemptEvidence` exact字段：`attempt`、`outcome`、`side_effect_state`、`completion_observed`、`http_status`、`retry_after_ms`、`input_tokens`、`output_tokens`、`cost_usd`、`cost_status`、`budget_charge_tokens`、`budget_charge_cost_usd`、`latency_ms`、`error_code`。结构化调用改用判别式`StructuredModelAttemptEvidence`，只在上述字段外增加必填`structured_output` exact摘要，禁止文本attempt携带该字段。 | `outcome=completed|retryable_status|failed|cancelled|unknown`；`side_effect_state=not_started|started|unknown`。未知字段为 null，不用零冒充。SDK exception、raw body、header、prompt、response id 和完整 URL 不得进入该结构。结构化摘要的逐值形状见MOD-005。 |
| `budget_charge` | `charged_tokens`、`charged_cost_usd`、`charge_status`、`unresolved_attempts` | `charge_status=actual|unknown`。逐维度穷举规则：`not_started` 为零；`started` 且有可信 usage/cost 纳入 actual 聚合；`started` 且任一已启用维度无可信 actual 时，无论 `completed`、确定性 `failed` 或 `retryable_status`，都保持原调用 reservation、标为 unknown/needs-review 并阻止 terminal，不得用上界替代 actual 或退款；side-effect unknown 遵循相同未决规则。Cost disabled 不参与该维度判断。不可能/矛盾组合或 actual-over fail closed，impact 遵循 `max(original_reservation,trusted_actual)`。调用只建立一个 reservation；仅当全部 started attempts 的所有启用维度 actual 可得且无异常时，才按聚合 actual 原子替换 reservation、退款并允许 completed/failed terminal。 |

嵌套字段类型固定如下：

- `route`：除下述显式条件可空字段外，ID/ref/version/origin/capability/schema 字段为非空 string；`endpoint_policy_digest` 与 `model_catalog_digest` 均为 64 位小写十六进制。真实 route 的 `credential_ref`、`model_catalog_ref/version/digest` 与 `request_shape_ref/version` 必填，fake 的真实网络字段可为 null。真实 route 的模型目录 identity 必须逐字命中 `ModelSettings.model_catalogs` 的同一 canonical entry。`text_completion|text_stream|structured_output` 的 `request_shape_ref/version` 固定为 `single-user-text-no-tools`/`v1`，且不得携带 tool catalog identity；`tool_intent` 固定为 `single-user-text-with-tool-catalog`/`v1`，并必须携带与 MOD-006 `tool-intent-request-identity-v1` 一致的 64 位小写 `tool_catalog_digest`/`tool_request_identity_digest`、非 bool 非负 `tool_catalog_utf8_bytes`/`max_tool_catalog_utf8_bytes`，actual bytes 不得超过 max。Deployment 的 allowed model 只保存 exact catalog ref/version，不能另行声明权威 envelope/price。`completion_classifier_ref/version` 必须同时为 null 或同时为非空 string；非空时 exact identity 仅为 `trusted_response_header_not_started`/`v1`，且必须由当前 endpoint policy/version 授权并计入 digest；`tool_intent`两项必须均为null。真实 route 的 `input_bound_strategy_ref` 固定为 `utf8-bytes-plus-envelope`、`input_bound_strategy_version` 固定为 `v1`，且必须与同一模型目录 entry 匹配；`input_envelope_token_bound`、`prompt_utf8_bytes`、`trusted_input_token_bound`、`output_token_cap`、其他 token bounds、`max_attempts` 与 timeout 均为非 bool integer，其中 envelope/prompt/bounds `>=0`、output cap/attempt/timeout `>=1`。No-tools capability 的 deployment 静态 `max_per_attempt_token_bound=max_prompt_utf8_bytes+catalog.input_envelope_token_bound+max_output_tokens`；`tool_intent` 则为 `max_prompt_utf8_bytes+catalog.max_tool_catalog_utf8_bytes+catalog.input_envelope_token_bound+max_output_tokens`。具体 route 的 no-tools `trusted_input_token_bound=prompt_utf8_bytes+catalog.input_envelope_token_bound`，tool-intent 则为 `prompt_utf8_bytes+tool_catalog_utf8_bytes+catalog.input_envelope_token_bound`；动态 `per_attempt_token_bound=trusted_input_token_bound+output_token_cap`、`reserved_token_bound=per_attempt_token_bound*max_attempts`。Cost enabled 时两项价格为有限非负 Decimal且`price_source_ref/version`非空；no-tools静态 `max_per_attempt_cost_bound=(max_prompt_utf8_bytes+catalog.input_envelope_token_bound)*input_price+max_output_tokens*output_price`，tool-intent静态公式在输入项中再加`catalog.max_tool_catalog_utf8_bytes`；两类动态公式统一为`per_attempt_cost_bound=trusted_input_token_bound*input_price+output_token_cap*output_price`，`reserved_cost_bound=per_attempt_cost_bound*max_attempts`。Cost disabled 时价格/source、静态/动态/reserved cost均为null。所有公式使用 checked arithmetic，动态值不得超过静态 ceiling；capability/request-shape/tool identity/catalog ref/version/provider/model/strategy/price/digest 未知或不匹配、deployment 复制值低报或高报、任一公式不一致、prompt/catalog/output 超 cap、价格或其来源 identity 缺失、非有限或溢出均在 reservation/client/network 前拒绝。Route、`budget-tree-v2` snapshot、operation identity 和 evidence 冻结同一目录及tool request identity与解析值，恢复不读取 current catalog/Registry 补齐或改价。Adapter 必须把 `output_token_cap` 逐字传为 Pydantic AI `ModelSettings.max_tokens`。`retry_policy` exact shape 为 `{retryable_http_statuses: integer[], max_attempts: integer, max_wait_ms: integer, backoff_initial_ms: integer, backoff_max_ms: integer}`；状态码去重、升序，且每项只能是 `429` 或 `500..599`，其余数值非 bool、非负，`max_attempts>=1`。未绑定 classifier 时 `retryable_http_statuses` 必须为空；绑定时只有 transport 从当前 endpoint 的 exact 单值 `X-Agent-Harness-Completion-State: not-started` 得到可信 false 且不存在 response id/usage/partial result，才可按该数组 retry。缺失、重复、多值、畸形、不可信来源或 body/exception 伪造均为 unknown/no-retry。`bulkhead_policy` exact shape 为 `{scope:"process_deployment", max_in_flight: integer, queue_timeout_ms: integer}`，两个整数均非 bool 且 `>=1`。
- `attempts[]`：`attempt` 为从 1 开始、严格连续递增的非 bool integer；`outcome` 与 `side_effect_state` 使用上表封闭枚举；`completion_observed` 为 boolean，完成状态不明时为 null；`http_status` 为 `100..599` integer 或 null；`retry_after_ms`、token、`latency_ms`、`budget_charge_tokens` 为非 bool 非负 integer 或按不可用规则为 null；cost 字段为有限非负 number 或 null，且继续满足 `cost_status` 组合；`error_code` 为稳定非空 string 或 null。not-started 的 attempt charge 为 0；actual 可得时为该 attempt actual；任一启用维度未决时该维 charge 为 null，不把 reservation 上界填入 attempt charge。
- `budget_charge`：`charge_status` 仅为 `actual|unknown`；`unresolved_attempts` 是去重、升序的正整数 attempt ordinal 数组。`actual` 要求数组为空，`charged_tokens` 为聚合非负 integer，cost-enabled 时 `charged_cost_usd` 为有限非负 number、否则为 null；`unknown` 要求数组非空，任一未决维度的对应 charged 字段为 null，已知部分 actual 只保留在 attempts 中，原 reservation/更高 impact 由 private ledger 保持而不伪装成 public charge。

只有所有已开始 attempts 都有可信实际 token usage 时，顶层 `input_tokens`/`output_tokens` 才填写全部 attempts 的实际和；任一缺失时必须为 null，不能只填写最终成功 attempt。Cost 全部可信时按 `reported|estimated` 规则填写聚合值，否则保持 null/`unavailable`。已知部分 actual 只留在 `attempts[]`，不得据此释放未决维度；顶层字段必须与 `decision.budget_charge` 对账。
...
### MOD-002 受控真实非流式文本模型运行时

| 字段 | 约束 |
|---|---|
| 状态 | 已同步主规格、归档并由本地提交 `ff0c49b` 交付。同一冻结身份上的 fresh Reviewer 1、Reviewer 2、Reviewer 3 均完成 Stage 1/2 PASS，0 findings；AC-077、078、079、080、082、084 的离线行为与安全边界已闭合。AC-081、AC-083 的真实托管 completion 仍为 `hosted-unverified`：两次授权 MiMo 入口分别为零调用与 side-effect unknown，不伪造 live PASS。未 push、发布或部署。仅覆盖单 provider kind 的 text completion，不包含 streaming、structured output、tool calling、多 provider 路由、Phase 21 重构或生产流量切换。 |
| 入口 | API、worker 与 CLI composition 统一从 typed settings 和 AgentRegistry 构造 provider-neutral router；业务 agent 不直接 import `pydantic_ai`、`openai` 或 SDK client。 |
| DTO / route | `ModelRequest` 不声明 endpoint/credential override 且 `extra=forbid`；未声明字段返回 `model.request_invalid`。Router 只对 deployment/model/provider assertion/capability 做缩权；只要 settings 含非 fake 受控 deployment，缺少 Agent model policy 就必须返回 `model.route_not_allowed`，不得回退 legacy/fake route。`provider_kind`、Agent/request provider assertion、route/evidence `provider` 和 adapter `provider_id` 在 `MOD-002` execution seam 使用同一 `fake|openai-compatible` identity，`pydantic-ai` 不属于公共 identity。未显式选模时，fallback 只按冻结 Agent 顺序从 deployment fallback 与 deployment∩Agent allowlist 的交集中选择；request 显式选模后不得自动改成另一个模型。`ModelDecision` 以 `estimated_tokens`/`max_tokens`、可选 `estimated_cost_usd`/`max_cost_usd`、`action`、`fallback_model` 与安全 `reason` 记录实际候选的估算、阈值和动作；decision/retry/bulkhead 都是深冻结 typed DTO，调用方不能在 eligibility 后修改嵌套执行边界。失败分别使用 `model.route_not_allowed` 或 `model.capability_unsupported`。 |
| 执行顺序 | immutable route 动态 hard eligibility（prompt/output/model-catalog/公式）→ soft policy/fallback/approval 与耐久 `policy.decision` audit → 调用级预算/settlement reservation → provider-neutral Bulkhead permit → lazy client factory 获取/构造绑定 frozen route 的 client lease → durable `side_effect_started` → adapter send。Policy check 固定为 bound execution identity、`action=model.invoke`、`resource=agent:<agent_id>:model`，context 只含 tenant/run/agent/request/trace、冻结 route/catalog identity、reservation bounds 与 soft-limit decision；其 `audit_ref` 是唯一授权成功审计，不新增 CanonicalEvent，也不占 event capacity，且只证明策略判断，不证明 provider 已开始。Soft policy deny、approval-required 未获批准和 hard eligibility 拒绝时，reservation/permit/client/mark/network 均为零；approval 只能缩权且不得提高、重置或覆盖 shared hard limit。Client 构造不得联网，失败按 not-started 回滚 reservation/permit；队列超时为 `model.bulkhead_saturated` 且零网络；mark 后崩溃、含糊 timeout/cancel/连接错误为 `model.provider_side_effect_unknown`/`needs_review`，不得自动重放。 |
| endpoint policy | `ModelSettings.endpoint_policies` 是唯一受信 endpoint policy catalog，mapping key 为 canonical policy ref，entry 冻结 policy version、适用 `provider_kind`、exact allowed origins、允许的 completion classifier identities，以及可选 provider-default endpoint catalog ref/version。它只从品牌 typed config 边界装载，Agent/request/provider ambient env 不得创建或修改 policy。Deployment 的 ref/version、provider、origin、classifier 与 default catalog identity 必须逐值命中一个 entry；未知、版本不匹配或越权组合在 storage/queue/client/DNS/HTTP 前 `config.invalid`。省略 `base_url` 时，只能由 `config/model_endpoints.py` 的版本化只读 provider-default catalog 解析；不得采用 SDK/env 默认。 |
| model catalog | `ModelSettings.model_catalogs` 是输入上界与价格的唯一受信 typed catalog，entry 由 canonical ref/version 标识并冻结 provider kind、model、request shape ref/version、`utf8-bytes-plus-envelope`/`v1`、非负 envelope bound、cost-enabled identity、input/output price、price source ref/version 与 canonical digest。MOD-002 文本调用唯一允许 `single-user-text-no-tools/v1`，该 shape 不定义且不得携带 `max_tool_catalog_utf8_bytes`；其他 capability 只有在对应 module contract 明确新增 shape、typed 字段和预算公式时才可使用。Cost enabled 时两项价格为有限非负值且 price-source identity 必填；cost disabled 时两项价格和 price-source identity 必须同时为 null，不得当作零价或沿用陈旧价格来源。Deployment 的 allowed model 只引用 catalog entry，不得独立自报 envelope 或价格；ref/version/provider/model/request-shape/strategy/price/source/digest 任一未知或不匹配均在 reservation/permit/client/network 前 `config.invalid` 或 `budget.reservation_rejected`。Route、snapshot 与 budget identity 必须冻结 catalog ref/version/digest 和解析值，reload 只影响新 root。解析与 digest 逻辑唯一位于 `config/model_catalog.py`；fake 保留既有离线零成本 route，不可借 fake 目录进入真实 composition。 |
| policy / approval | `PolicyEngine.evaluate(PolicyCheck)` 的三态结果与 `AuditService` `policy.decision` 记录复用既有 seam。`require_approval` 必须创建既有 `policy_approval` checkpoint 与 `ApprovalRecord`，保存 `AgentApprovalRequest(action=model.invoke, resource=agent:<agent_id>:model, arguments_hash=canonical ModelRequest hash)`；未批准不建立 model reservation。ApprovalService 只能从 durable record/lease 构造 `ApprovalGrant`，runtime 先校验 approval/lease/tenant/identity/agent/run/action/resource/arguments hash、状态与单次 lease，再调用 `BoundModelInvocationService.complete_approved()`；后者不接受调用方 bool，只跳过同一 soft decision并重新执行 hard route/catalog/current owner balance，稳定 operation identity 绑定 approval id。Mismatch、stale、duplicate/replay fail closed；获批 continuation 最多调用 provider 一次，crash/replay 只恢复既有 settlement。既有 `approval.required|resolved`、`run.resumed` 和 policy audit 承载证据，不新增 model authorization event。 |
| retry | 默认/官方 endpoint 不提供可信 completion signal，`retryable_http_statuses` 必须为空。只有 endpoint policy/version 绑定 `trusted_response_header_not_started/v1`，且私有 transport 从当前 endpoint 取得 exact 单值 `X-Agent-Harness-Completion-State: not-started`、无 response id/usage/partial result 时，才可把 policy 列明的 429/5xx 在同一 permit/reservation/client lease/mark 内 retry。缺失、重复、多值、畸形、非绑定来源、body/exception 伪造或已观察结果均 unknown/no-retry；等待同时受单次上限、attempt 上限与 total deadline 约束，超限返回 `model.provider_retry_exhausted`。 |
| budget | 真实 route 先解析唯一受信 model catalog entry，用其中认证的 `utf8-bytes-plus-envelope/v1`、envelope 与价格计算 input bound，并冻结 request 只能缩小的 output cap；deployment 不得提供另一套权威 envelope/price。Per-attempt token/cost 与调用级 reservation 必须按 5.29 公式 checked 计算；默认候选超过冻结 soft threshold 或 target hard limit时，每个 fallback 都必须用自己的 catalog/envelope/price/attempt bound 重算并再次比较，全部不合格时只产生 `policy_required`，不得预约或调用默认模型，approval 也不得绕过 hard limit。Catalog unknown/mismatch/digest 漂移、fallback 冻结公式损坏、配置复制值低报或高报、prompt/output 越界、strategy/价格缺失或溢出均在 reservation/client/network 前拒绝，adapter 把同一 output cap 强制传给 provider。逐维度按穷举矩阵处理：not-started 为零；started 且有可信 usage/cost 纳入 actual 聚合；started 且任一启用维度 actual 不可得的 completed、确定性 failed、retryable status，以及 unknown/非法/actual-over，均保留原调用 reservation 或更高可信 actual impact并进入 needs-review。只有全部 started attempts 的启用维度 actual 可得且无异常时才原子结算、退款并允许 terminal，不得只计最后一次。 |
| snapshot | 新 root 使用 `schema_version=budget-tree-v2` 与 `budget-tree-v2:` id，私有 payload 冻结每条真实 route 的 canonical base URL/path、endpoint policy ref/version/digest 与 completion classifier ref/version；公开 evidence 只投影 origin/identity。完整旧 `budget-tree-v1:`、无 schema version 且 provider=fake 时仅从旧 payload 投影 synthetic legacy fake route；partial、混合版本、真实 provider v1、缺 endpoint path/classifier identity 或需当前配置补齐的快照返回 `budget.reservation_rejected`/`snapshot_invalid`，零 provider 副作用。 |
| stable errors | `model.request_invalid`、`model.route_not_allowed`、`model.capability_unsupported`、`model.bulkhead_saturated`、`model.provider_failed`、`model.provider_retry_exhausted`、`model.provider_side_effect_unknown`、`model.invocation_cancelled`、`budget.reservation_rejected`；配置继续使用 `config.invalid`、`config.secret_file_conflict`、`config.secret_file_invalid`。durable mark 前的取消可用 `model.invocation_cancelled`，mark 后默认使用 unknown 语义；MOD-003 route-chain stream只有在provider-neutral关闭结果证明`stopped + complete usage`且无durable delta不确定性时，才允许以可信actual的`model.invocation_cancelled`终止，仍不构成completed或selected。`ModelProviderInvocationError` 不串联 raw adapter/SDK 异常，只公开稳定 code 与非负且一致的 `provider_called`、`attempt_count`、nullable `latency_ms` 安全摘要；流式 live executor 另消费封闭 in-process `failure_domain=provider|runtime` 区分外部故障和本地 contract failure，不把 raw exception、SDK 类型或该内部来源写入 artifact。 |
| evidence | 遵循 5.29 的 `route`、`attempts[]` 与 `budget_charge`；不得记录 prompt/response 全文、raw provider payload、header、secret、完整 URL、SDK exception/client。live smoke 在 provider 失败时必须使用 invocation 的安全副作用摘要，不能因没有成功 response 而把 429/5xx、retry exhausted 或 timeout/unknown 误报为零调用；相同 usage call 的稳定失败（含零调用的 bulkhead saturation）先按 error identity 从 exact `error_code/provider_called/attempt_count/latency_ms` failure 恢复同一事实，绝不再次调用 provider。公开调用重放与后台恢复必须先共用完整 settlement validator 校验 durable started identity、final evidence、outcome、failure/response，再允许发布 final event/telemetry 或标记 outbox `published`。任一 outcome 只要出现 `attempts[]` 或 `budget_charge`，就必须成组提供两者及 exact typed route；final route 必须逐字段匹配 started route，且 provider/model identity、endpoint/catalog digest、request/input strategy、timeout/retry/bulkhead 以及 token/cost/reservation 公式必须先逐值成立；classifier 为空时 retryable response status 必须为空。`completed` 必须有同 provider/model、非 `policy_required` action 的 response 且 error code 为空；`model.policy_required` rejection 必须有匹配 response，hard budget rejection 不得有 response。稳定错误与 response 共存、字段缺失/多余、call/attempt 矛盾、非法 latency、畸形 evidence/outcome 或 response 全部统一为 replay error，不得发布损坏结果、返回成功或泄漏普通/Pydantic 校验异常。 |
| client / ambient env | Composition 只注册私有 lazy `ControlledOpenAIClientFactory`，startup 不预构造真实 SDK client。锁定 `openai==2.44.0` 会检查多种 `OPENAI_*` env，因此 factory 显式传 typed key、空 admin/org/project/webhook、冻结 base URL/空 default headers，并使用不修改进程环境的 `trust_env=False`/no-redirect client 与出站 allowlist transport；transport 在 socket send 前 exact 校验 endpoint policy catalog 已批准的 origin/path，并从 typed secret/adapter 常量/attempt identity 重建 Authorization 与封闭 header，删除 ambient/SDK 合并的 identity/custom headers。 |
| offline / live | 默认 quality/unit/contract/eval/smoke-local 只使用 fake/double，忽略 provider 原生 ambient key/admin/org/project/webhook/custom-header/base-url/proxy，并以 typed-client/transport double 与网络哨兵证明它们不能改变出站身份或联网。真实 smoke 固定通过 `make smoke-live-model` / `make ci-smoke-live-model`，需要单独授权、显式 opt-in、隔离 credential 与受信 endpoint。无授权或本地条件不全输出 `model-live-smoke/v1 status=hosted-unverified`、进程 0，并映射为 `ci-result/v1 status=skipped`；已授权但外部条件阻塞输出 `external-blocked`、进程 2并映射 CI fail，其中 `provider_called`、`attempt_count`、`latency_ms` 必须按安全失败证据如实填写；真实成功才是 live pass。 |
| validation | SQLite 与真实 PostgreSQL 覆盖 v1/v2 snapshot recovery、预算结算、幂等与 crash windows；provider/client/transport double 覆盖 provider identity、全部已知 SDK ambient env 隔离、startup client count=0、unknown/mismatched endpoint/model catalog 在 storage/queue/reservation/client/DNS/HTTP 前失败、完整 hard eligibility→soft policy/fallback/approval+policy audit→reservation→permit→client→mark→send 顺序与逐边界失败、policy deny/approval-required 未批准的全部后续副作用为零、durable checkpoint/approval/grant 全绑定/单次 continuation/mismatch/stale/replay/audit、获批恰好一次 provider 调用、版本化 trusted completion classifier/default no-retry、prompt/output cap、token/cost 公式、adapter 强制 max_tokens、deadline/cancel/bulkhead/cleanup。独立 live smoke 只在另行授权时运行。 |

### MOD-003 受控跨 deployment/provider fallback

| 字段 | 约束 |
|---|---|
| 状态 | `controlled-multi-provider-failover` 的协议增量与 runtime 已完成最终门禁和 fresh `1+2` 实现审查，主规格已同步并归档。历史 live failover 只完成零调用 `hosted-unverified` 预检，未建立双真实 provider 成功；不得把该外部未验证状态或本模块历史交付冒充 Phase 19 结构化输出能力。 |
| 公开入口 | 不新增 HTTP endpoint。API、worker 与 CLI 继续通过既有 run/model invocation seam；本协议只扩展 typed Agent/deployment model policy、provider-neutral route plan/evidence 和 composition。 |
| 配置 | Agent policy 以有序 route refs 表达显式 `fallback_routes`，每个候选稳定身份为 `(deployment_id, model_id)`；缺少该字段的旧 descriptor 保持 legacy 单 deployment 行为，不隐式启用 route chain。Chain mode仍保留既有必填 model字段，但只作为请求缩权前原始 Agent `fallback_routes[0]`兼容投影：`provider`等于原始首 deployment provider kind，`deployment_id/default_model`等于原始首 ref，`allowed_models`只按原始 route顺序投影首 deployment中已列 models，`fallback_models=[]`；任何不一致关闭失败，投影不授权后继，也不随request改写。`provider_kind`、endpoint、credential ref、model catalog、capability、timeout/retry、默认空的 `cross_provider_failover_http_statuses`、Bulkhead 与价格只从每个候选自己的 typed deployment派生；跨 provider状态白名单只允许显式去重的 403、429或 500～599，非空时必须绑定受 endpoint policy/version允许的 `trusted_response_header_not_started/v1` 并进入 route/snapshot/retry-policy digest。403默认不启用；同一 provider kind的不同 vendor endpoint/credential仍视为不同 deployment。 |
| 请求 | Request 只能从逐候选 deployment∩Agent 已冻结 route chain 中选择或删减候选，不得借首候选兼容投影新增或重排 deployment/provider/model，不得声明 endpoint、credential 或 SDK adapter。单值 provider/deployment/model只允许缩权到一个唯一 ref；与 `route_refs` 并存时后者必须只有一个且逐值一致。请求删掉原始首 route时不能改写 Agent descriptor兼容投影或完整授权；缩权只由 candidates表达。非法字段继续使用 `model.request_invalid`，越权/空交集继续使用 `model.route_not_allowed`。 |
| Route plan | 首次 client、DNS、HTTP 或预算副作用前冻结完整 route chain、chain identity、候选 ordinal，以及各候选 endpoint/catalog/credential identity、capability、deadline/retry/Bulkhead 与 token/cost 上界。`chain_id`只允许从 `model-route-chain-id-v1` exact preimage计算：capability、candidate count、请求缩权前原始 Agent descriptor的首-route兼容投影与完整有序 fallback refs、request的 prompt UTF-8 byte/max-output上限，以及缩权后的有序 candidate public identity；Agent `[A,B]`、request `[B]`仍以 A投影和 `[A,B]`写 `agent_model_policy`，只有 `candidate_count=1/candidates=[B]`。Cost bound以无 exponent/尾零/负零的 canonical Decimal string或 JSON null编码。`chain_id`、attempt identity、两类not-started proof与approval request/grant binding统一只用`model-route-canonical-json-v1`：UTF-8、`ensure_ascii=false`、排序键、紧凑separators、拒绝NaN；exact object的nullable字段不得省略。Current/snapshot、direct/allocation、SQLite/PostgreSQL、approval handoff与公开evidence validator复用active delta列明的六组逐字节golden vectors，其中含可区分`ensure_ascii=false/true`的合法非ASCII request向量；unknown/缺失字段、Unicode转义、bool数字、float/非有限值、nullable漂移、request/grant交叉绑定错误或任一权威字段变化均关闭失败。Public DTO/evidence只包含去敏identity，不包含完整URL、credential value、header、prompt内容或SDK对象。 |
| 调用顺序 | Legacy单route逐字保留`hard eligibility → policy/audit → reservation → Bulkhead permit → client lease → durable side_effect_started mark → send`。显式route chain唯一改为`hard eligibility → candidate policy/audit → candidate reservation → durable attempt_lifecycle started identity → Bulkhead permit → candidate-isolated client lease → send/iterate`；started必须在permit/client/send前提交。Permit/client/prepare在send前确定失败时，proof与同一lifecycle关闭原子提交；started提交后关闭前崩溃或commit-ack未知保留reservation/needs-review，不重取资源、不重发、不创建下一attempt或切provider。 |
| Failover 条件 | 静态 hard eligibility 可以在零 provider 副作用时跳过候选。每次首次调用或同 route retry都先在permit/client/send前耐久创建全局连续、不可覆盖的`attempt_lifecycle=started` identity，绑定chain、usage/operation、candidate、attempt及route/endpoint/retry digest；started record即使`request_sent=false`也禁止恢复重发。每个实际 attempt只接受两类推进事实：`client_not_started` 要求该 attempt 的 request/HTTP response均未发生；`trusted_business_not_started` 要求该 attempt 已发送且 request/HTTP response均已发生，但当前 endpoint policy/version 的唯一合法 classifier header、deployment 冻结状态白名单及无 response identity/usage/text/delta共同证明业务执行和计费未开始。下一retry/transfer前必须把proof与同一lifecycle的`started→not_started_proven`在一个UoW提交；候选只有在全部lifecycle records均已证明not-started且与proof一一匹配时才可跨provider。started mark后send前、send后proof/settlement前或commit-ack未知都保留reservation/needs-review，不创建下一attempt或后继provider。状态码/body/exception string 本身不受信；无证明的 write/read timeout、取消、response、unknown或任一生成/计量事实都停止。Harness `PolicyEngine` deny 始终终止，不能映射为 provider classifier。 |
| Budget / settlement | 每个候选按自己的冻结 catalog/envelope/price/attempt policy计算 reservation。固定顺序为 static eligibility后逐候选执行 policy：deny终止、require-approval暂停、soft review threshold选择有限 fallback则耐久记 `budget_ineligible/soft_budget`、allow才在 owner lock/CAS中尝试 current-balance reservation；余额不足耐久记 `budget_ineligible/balance`。两类 budget-ineligible均为零 attempt/proof/reservation/permit/client/provider，且不等于 deny/require-approval。Legacy单route/embedding identity逐字保持`budget-operation-v1`；显式chain强制`budget-operation-v2`，以ordinal 1 provider/model/price/bound兼容投影及等于`chain_id`的digest/candidate count参与hash，禁止跨版本、缺字段或随active/余额选择改写。Route chain在首次可信 bound entry、任何 policy/approval record前，以受信上下文和原始语义 operation key生成并冻结 64位小写 SHA-256 `usage_call_id`与 `operation_identity_digest`。首候选等待审批只以该 ID建立零 impact coordination claim，不建立 model reservation；waiting state保存绑定同一身份且不含 approval/lease的 `model-route-chain-approval-request-v1` digest。ApprovalService在独立 UoW创建绑定同一身份的 approval record、独占提交 resolution lease并生成 grant；invocation从私有 checkpoint重算并校验后，shared-budget UoW才以 request digest/usage call id CAS并重检目标 balance：足够时保存 grant digest并激活，不足时保存 grant digest和 budget-ineligible、保持零 impact，后继重新执行独立 policy且不得复用 grant。该获批balance-skip保留双binding和此前waiting transition，禁止本ordinal reservation activation；继续后继只允许零released impact的`transferred/balance` coordination transition。该 UoW不声称消费 lease；approval lease的 fencing/finalization仍由既有 ApprovalService/repository拥有。Chain续跑复用原 claim/settlement/outbox/stream group，禁止用 legacy `operation_key="approved:<approval_id>"` rekey、建立映射或第二 claim；legacy单 route审批行为不变。进入下一候选前，上一候选全部实际 attempts必须各自以两类可信 not-started之一和 durable proof record收敛；同一 UoW原子 actual-zero结算当前候选，跨过任意 static/budget-ineligible中间候选并把 reservation直接转移到首个 eligible后继，只产生一条 current→后继 transition。若没有 eligible后继，同一 UoW释放 current impact并提交 current→null exhausted terminal；初始全不可用链保持空 transitions/空 reservation。调用级 claim高水位、逐候选聚合状态与逐 attempt事实分别单调，末次 `client_not_started`不得把早期 trusted-business历史回写成未发送。同候选多 retry proof、两类 proof的三候选双顺序与预算 skip在 direct/allocation、SQLite/PostgreSQL及 crash replay中逐值一致。Policy deny不计入 exhausted cause。其他 started/无完整 proof的 HTTP response、unknown或悬空 attempt保留原 reservation并进入 needs-review或按可信 actual结算，绝不推进 ordinal。Replay先返回首次 durable budget决定，并在任何后继调用前从冻结 chain canonical bytes重算全部历史 attempt identity与not-started proof digest；即使篡改后的引用彼此一致、形状合法，只要与冻结chain重算值不一致也必须冲突并保持后继零调用。恢复只补同一 settlement或启动已经耐久转移、尚无自身attempt identity的后继一次，不因后来余额重选，不再次调用已耐久记录的 provider attempt。 |
| Streaming | 首个 delta尚未观察，且当前候选全部attempt lifecycle均已关闭为`not_started_proven`、与完整有序proof逐值匹配时才允许按冻结链切换；每次stream retry同样必须先创建started identity，下一retry前再原子关闭。已存在started identity、send前/后崩溃、commit-ack未知或数据库尚无delta都不得解释为可重发/切换。观察或提交任一 delta后，取消、deadline、SSE/CLI断线、`Last-Event-ID`、恢复和重连都不得触发跨 provider fallback。chain事件的公开 `attempt`使用实际产出候选的全局 provider attempt ordinal；legacy stream继续为 1，event id规则不变。 |
| Evidence | 在 5.29 provider-neutral evidence基础上保留既有 `route`、`attempts[]`、`budget_charge`，并新增 exact `decision.route_chain={"schema_version":"model-route-chain-evidence-v1","identity":<model-route-chain-v1>,"state":<model-route-chain-state-v1>}`。state冻结 usage call id、operation identity digest、candidates、transitions、selected ordinal、始终非空的 `evidence_route_ordinal`与去敏 failover cause，并为每个 candidate单调记录聚合 `side_effect_state`、request/response/observations、有序 `not_started_proofs[]`、`approval_request_binding_digest`与 `approval_grant_binding_digest`；原始 operation key不公开，两份审批字段只公开 digest，不公开 approval/lease identity。Transition exact table额外封闭`activated/initial`、两类`transferred/not_started`、`waiting_approval/approval_required`、`approved/approval_granted`、`transferred/balance`与两类`terminated`的from/to/released/reserved组合；成功审批只允许waiting ordinal→同ordinal、released=`0/null`、reserved=目标冻结bound并直接waiting→active，不得重复activated，余额不足不得出现approved/activated，并以该ordinal作为跨普通skip不变的零impact source anchor；其后allow/waiting/deny/exhausted分别使用balance transfer、零bound waiting、零bound policy-denied terminal或route-exhausted terminal。初始前导skip不成为source，后续allow/waiting从null开始，deny零transition；commit-ack不得追加或改写sequence。Candidate state精确为 `pending|static_ineligible|budget_ineligible|waiting_approval|active|not_started|completed|cancelled|unknown|denied`：static-ineligible只允许 `reason=static_ineligible`，budget-ineligible只允许 `reason=soft_budget|balance`；两者都只能是零 attempt/proof/reservation与全 false观察事实，不能伪装为 pending/denied/not-started。Static/soft-budget/普通 balance skip的 approval bindings均为空且不为该候选追加 transition；只有获批 activation的 balance skip强制 request/grant两个 digest同时非空，禁止单边 binding或 grant跨候选复用，并可保留既有 waiting coordination及后续零释放 `transferred/balance`，但不得出现为该候选激活 reservation 的 transition。`cancelled`只允许`reason=invocation_cancelled/side_effect_state=result_committed/completion_observed=false`、selected/active/waiting为空、current reservation为canonical空、evidence route ordinal命中当前candidate且不新增transition。调用级 claim的 started只表示整链高水位，不能覆盖候选或 attempt事实。`decision.route`及顶层 provider/model必须逐值映射该 evidence ordinal，而不能在 cancelled/denied/exhausted等未选择终态伪造 selected candidate。公开 `model.request.started`是 provider副作用前的调用生命周期 evidence，本身不固定最终 route。不新增顶层 `ModelUsageEvidence.usage_call_id`；唯一新增公开位置是 `decision.route_chain.state.usage_call_id`，并与 CanonicalEvent/telemetry correlation逐值相同。Chain-mode每个实际 attempt在既有字段外逐项记录 candidate/deployment/provider/model、request/HTTP-response/status、response-identity/usage/text/delta观察位、completion、endpoint/classifier identity与 nullable not-started reason/proof digest；零 charge attempt必须和 candidate proof record逐值一致并可重算 digest。 |
| Attempt lifecycle | `model-route-chain-state-v1.attempt_lifecycle[]`按global attempt从1连续；每项exact保存attempt、candidate ordinal、不可变attempt identity digest、`started|not_started_proven|unknown|settled`、side-effect与request/response/result观察位及nullable proof digest。Identity digest使用`model-route-attempt-identity-v1` exact object经统一canonical serializer计算，绑定chain/usage/operation/candidate/attempt/route/endpoint/retry并匹配active delta golden vector；两类proof和approval request/grant binding同样必须匹配各自exact bytes/hash。Started必须在route-chain permit/client/send前提交；仅允许同attempt原子关闭到三个终态，终态不可降级。`not_started_proven`与candidate proof、公开attempt evidence一一匹配；unknown/settled无proof。任何started悬空或commit-ack未知都保留reservation并needs-review，不重发、不创建下一attempt、不跨provider。 |
| 取消结算 | `prepared`只证明本地资源所有权，不提升`request_sent/provider_called`。Route-chain stream显式取消/deadline仅在close result逐值证明`state=stopped`、usage `finality=complete`且input/output与启用cost维度完整，同时完整durable stream group无delta intent、`result_persisted`前缀或发布确认不明时，才在同一owner UoW按actual替换reservation：matching lifecycle=`settled/result_committed/completion_observed=false`，candidate=`cancelled/invocation_cancelled/result_committed`，claim=result_committed，active/waiting/selected为空且current reservation为canonical空；不新增transition、不发布`model.output.completed`、不fallback。Final attempt=`outcome=cancelled/side_effect_state=started`，budget charge为actual，error=`model.invocation_cancelled`。Usage不完整、close unknown/失败、not-started或durable delta不确定性都保留reservation/capacity并needs-review；无明确request/response/result/usage/text/delta观察时`provider_called=false`且attempt identity仍计数，cleanup失败不得覆盖稳定安全错误。 |
| 错误 | 除完整 `trusted_business_not_started` 外，已开始、出现 HTTP response、含糊 timeout 或 unknown 的候选沿用 `model.provider_side_effect_unknown`/原稳定 provider error并停止。所有候选均以 `static_ineligible|budget_ineligible|client_not_started|trusted_business_not_started` 安全耗尽时返回 `model.route_chain_exhausted` 与封闭 cause summary；static映射 `capability|catalog|input_bound|hard_budget`，budget映射 `soft_budget|balance`，有实际可信 attempt才映射 `not_started_failure`。Chain恢复的公开安全摘要中，`attempt_count`统计已耐久的全局attempt identity数量，`provider_called`仅在任一attempt已有request、HTTP response、result、usage、text或delta观察事实时为true；因此send前已耐久started identity可以合法产生`provider_called=false`且`attempt_count>0`。该例外只适用于chain恢复摘要，不放宽既有持久化provider失败证据校验。`model.policy_denied`与 approval waiting独立返回，不得混入 exhausted cause。任何公开 envelope 都不得包含 raw provider error、endpoint、credential、余额或 hard limit。 |
| Fake / 离线 | Fake 只能作为 profile/Agent 显式 route，不能是全部真实候选失败后的隐式尾项。默认 quality/test/eval/smoke-local 继续零网络。`model-failover-live-smoke/v1`是 exact四分支联合：顶层字段只允许`schema_version/status/provider_called/attempt_count/chain_id/selected_ordinal/candidates/usage/reason_code`；candidate额外逐项保存非bool attempt/proof count，ordinal从1连续唯一且顶层`attempt_count`等于其总和；usage token为非bool非负整数，cost status只允许`reported|estimated|unavailable`且cost为有限非负number或null、null当且仅当unavailable。验收探针固定两个不同`deployment_id`、两个隔离`credential_ref`、两个不同受信endpoint和两条`max_attempts=1` route，二者`provider_kind`可以相同；PASS唯一允许ordinal `[1,2]`、`attempt_count=2`、`selected_ordinal=2`、首项not-started/1 proof、次项唯一completed/1 attempt和同一durable evidence逐值一致，并包含同`provider_kind`但不同deployment/credential/endpoint的正合同。`reason_code`按status封闭：passed只允许null；hosted按固定优先级只允许`authorization_missing|failover_opt_in_missing|credential_pair_missing|deployment_pair_invalid|not_started_fixture_missing`；external只允许`network_unavailable|provider_rejected|quota_blocked|provider_timeout|provider_result_unknown`；failed只允许`contract_failure`。前置缺失的hosted分支必须`provider_called=false/attempt_count=0/chain_id=null/selected_ordinal=null/candidates=[]/usage=null`且零网络；chain冻结前`failed/contract_failure`只允许相同空identity但进程1，除此之外的passed/external/failed均须非空chain与两个candidate，failed空/非空identity不得混搭。完整前置后的外部故障才是external-blocked。unknown/重复/零ordinal、`selected_ordinal`不存在、重复completed、attempt/proof count冲突、token/cost非法、reason/status不匹配或artifact/evidence不一致都关闭失败，required acceptance不得记PASS。 |
| 非目标 | 动态 provider discovery、健康权重、流量分配、自动成本套利、热重载控制面、多区域运维、跨 provider 账单对账、structured/tool-call failover 和 provider raw DTO。 |

### MOD-005 Provider-neutral Structured Output

| 字段 | 约束 |
|---|---|
| 状态 | 本合同定义provider-neutral structured output的稳定公共入口、字段、顺序、错误、兼容与恢复语义；交付验证、审查和归档等过程状态只在OpenSpec tasks与living plan维护。 |
| 可信入口 | `BoundModelInvocationService.complete_structured(request, operation_key=..., repair_limit=...) -> ModelResponse` 是业务 executor 的首次结构化调用入口；它复用 bound identity/tenant/run/agent/request/trace 与既有 usage call identity，并在总reservation、client和send前执行既有`model.invoke` Policy/HITL。DENY以`model.policy_denied`零调用结束；REQUIRE_APPROVAL只返回durable waiting并冻结原usage/operation/schema/repair identity与同时绑定usage/operation/request/schema/repair的arguments hash。`complete_structured_approved(request, operation_key=..., repair_limit=..., grant=...)`是唯一批准恢复入口：operation key不得重派生identity，服务从durable record/checkpoint恢复exact continuation、校验active grant/lease与完整arguments绑定，只绕过一次soft gate并重算hard route、capability、prompt、余额与联合reservation。底层非 bound `ModelInvocationService` 只供 composition/recovery，不是业务扩权入口。普通 `complete/complete_approved` 与 `stream/stream_approved` 签名和 text-only 行为保持不变。 |
| Structured approval identity | arguments preimage为`structured-policy-approval-arguments-v1` exact object，字段固定为`schema_version/usage_call_id/operation_identity_digest/request/schema_identity/repair_limit`；两个identity为64位小写SHA-256，`request`固定保留`deployment_id/provider/prompt/model/capability/estimated_input_tokens/max_output_tokens/timeout_seconds/route_refs`九个键及显式null，route ref固定为按原顺序排列的`deployment_id/model_id` exact object。使用`structured-canonical-json-v1` bytes取SHA-256。Continuation为`structured-policy-approval-continuation-v1` exact object，字段固定为`schema_version/kind/usage_call_id/operation_identity_digest/schema_identity/repair_limit/arguments_hash`，其中`kind=structured_policy_approval`。Approval metadata与checkpoint分别保存逐值相同的continuation，metadata的`arguments_hash`、grant hash及从continuation identity加当前bound request/schema/repair重算的hash必须一致；extra/missing/type drift，usage/operation/request/schema/repair/grant/lease的单项或组合篡改，以及只同步改写record/checkpoint两份continuation identity，均在provider前关闭失败。 |
| Schema identity | `OutputSchemaIdentity` exact fields 为 `schema_version="output-schema-identity-v1"`、非空 `schema_ref`、非空 `version`、64位小写 SHA-256 `digest`。Registry 在同一次全量 Agent 加载中解析 `output_schema`，以 descriptor `version` 作为 schema version，对严格 canonical JSON Schema 计算 digest并保存只读 catalog；descriptor只公开 identity，不公开 Python class或adapter类型。Schema reference/version/digest/definition任一不一致时 Registry整体拒绝或调用以 `model.structured_schema_unknown|model.structured_schema_conflict` 关闭。 |
| Canonical schema | Canonical JSON Schema 只接受根 `type=object`，所有 object 层显式 `additionalProperties=false`，拒绝 remote `$ref`、递归/未解析 `$ref`、非JSON值、NaN/Infinity与不受支持关键字；UTF-8、排序键、紧凑 separators、`ensure_ascii=false` 形成唯一 bytes。Schema对象位置的允许关键字精确为引用/结构`$defs|$ref|type|properties|patternProperties|additionalProperties|required|items|prefixItems`、值约束`enum|const|minimum|exclusiveMinimum|maximum|exclusiveMaximum|multipleOf|minLength|maxLength|pattern|minItems|maxItems|uniqueItems|minProperties|maxProperties|dependentRequired|anyOf|oneOf|allOf|not`和annotation `title|description|default|examples`。递归schema位置只包括`$defs/properties/patternProperties`的mapping values、`items/not`的object value与`prefixItems/anyOf/oneOf/allOf`的array object items；mapping业务keys、annotation/default/example/enum/const及普通constraint值不按schema遍历，boolean schema拒绝，`additionalProperties`只允许false。`format|contains|minContains|maxContains|if|then|else|dependentSchemas|propertyNames|unevaluatedProperties|unevaluatedItems`及其他schema位置关键字关闭失败。业务 Pydantic model仅是 Registry 启动时的 schema 来源，不能进入 SDK核心 DTO、request、response或持久化 evidence。 |
| 请求 | `StructuredOutputRequest` exact fields 为 `schema` identity 与非bool整数 `repair_limit=0..2`。可信 bound façade只允许使用当前 Agent已注册的 output schema；调用方可以进一步缩小 repair上限，不能提交另一 schema definition、提高 deployment上限或改变 provider/route。未知schema、identity冲突、repair-policy越权分别在usage claim/reservation/client前以`model.structured_schema_unknown`、`model.structured_schema_conflict`、`model.structured_policy_invalid`零调用拒绝。请求 capability固定为`structured_output`，只允许 legacy 非流式单 route；Agent policy只要显式声明任意非空`fallback_routes`就保持Phase18.2 route-chain identity，即使request缩权后只剩一个candidate也以`model.structured_route_not_allowed`在usage claim/reservation/attempt/client/send前拒绝，不得把显式chain降级为legacy。实际provider字符串固定为`structured-provider-prompt-v1` exact canonical object：`schema_version/phase/repair_ordinal/business_prompt/schema_identity/schema/validation_codes`；initial为ordinal0/空codes，repair只携带排序去重稳定codes，不含path或invalid原文。Structured下`max_prompt_utf8_bytes`约束完整字符串，planning零调用构造initial及各repair ordinal的最大code集合prompt逐一检查。 |
| 响应 | `ModelResponse.output_text` 保持必填。纯文本 response 的 `structured_output=null`；结构化成功时 `output_text` 等于 value 的 canonical JSON 文本，`structured_output` exact判别为 `schema_version="structured-output-result-v1"`、schema identity、`status=valid`、JSON object `value`、`repair_count`、`provider_request_count`、64位 replay identity。Invalid/exhausted/unknown 不返回伪成功 value；失败通过既有稳定 invocation failure及durable evidence收口。 |
| Replay identity | `structured-output-replay-v1` exact keys固定为`schema_version/tenant_id/run_id/agent_id/request_id/trace_id/usage_call_id/operation_identity_digest/prompt_digest/deployment_id/provider/model/route_digest/schema_identity/transport_attempt_limit/repair_limit/repair_count/provider_request_count/final_status/value_digest`，nullable字段也必须出现。Final status仅`valid|invalid|extra_fields|repair_exhausted|failed|needs_review`，value digest仅valid非null；各摘要为64位小写SHA-256，整数拒绝bool并满足冻结上限。确定终态的repair/request count必须是exact整数；`failed`允许request count为0，表示claim后的client/prepare发送前失败、每个transport attempt均有核心发送前proof的retry耗尽或确定取消。只有`needs_review`允许repair/request count为显式null，分别表示实际repair轮次或provider request基数无法从耐久事实证明；已知维度仍保存精确非负整数，不能用0/1代替unknown。`structured-canonical-json-v1`固定UTF-8、`ensure_ascii=false`、排序键、紧凑separators、显式null和有限JSON number，拒绝Decimal/bytes/NaN/Infinity/unknown/缺失；operation/prompt瞬时preimage、route/value算法与golden vector由OpenSpec主capability逐值冻结。零调用preflight错误不创建usage claim并固定null replay；durable started后的final/needs-review必须非null。相同调用只接受exact durable result/failure并只补投；schema/repair/prompt/route任一语义变化以replay conflict零调用拒绝。 |
| Capability / adapter | typed deployment显式声明`structured_output`，对应 provider实现 provider-neutral structured protocol。Pydantic AI/OpenAI类型、provider-native schema/output对象只能存在于`adapters/models/`；核心给adapter的只是 canonical JSON Schema与字符串prompt，adapter返回JSON-compatible object、provider-neutral usage/attempt。Adapter不得注册/执行工具，也不得把tool output或provider tool call当业务结构化结果。 |
| Provider candidate / protocol | Adapter到核心validator之间唯一返回类型为`structured-provider-candidate-v1` exact `StructuredProviderCandidate`：`schema_version/schema_identity/provider/model/candidate/attempts`。`candidate`只允许原始JSON字符串或递归JSON object；DTO不得重复携带顶层token/cost/latency，attempts必须恰含一个`attempt=1`的provider-local `ModelAttemptEvidence`，作为本次single-request send的usage/cost/latency/side-effect/outcome唯一来源。该DTO只在进程内使用，不进入descriptor/HTTP/durable payload；原始candidate不进入repair prompt/异常/evidence/log，identity/provider/model漂移或SDK/裸tuple/object/`ModelResponse`旁路均fail closed。`ModelStructuredProvider.prepare_structured(request: ModelRequest, *, plan: ModelRoutePlan, schema: OutputSchemaDefinition) -> PreparedStructuredModelCall`、prepared call的async `send_structured(*, provider_prompt: str, repair_ordinal: int, transport_ordinal: int) -> StructuredProviderCandidate`与`aclose() -> None`是唯一签名；核心为每对repair/transport ordinal建立fresh prepared call并负责受保护cleanup，prepare零发送，send最多调用一次且恰好执行一次外部transport request，SDK/client/adapter不得隐藏retry、backoff或repair。核心公开`StructuredProviderPrepareError(retryable)`与`StructuredProviderCallError(code, attempts)`为唯一异常出口；后者只允许`model.provider_failed|model.provider_side_effect_unknown`和一个local attempt，adapter私有错误不得越界。 |
| Repair / budget | Deployment冻结 `max_structured_repair_attempts=0..2`，请求只能缩小；`provider_request_limit=transport_attempt_limit * (1+repair_limit)`。核心协调器显式拥有repair×transport双层循环、单一绝对deadline、backoff、retry分类与次数推进；每个transport ordinal重新prepare，只有显式retryable prepare错误允许发送前proof与下一ordinal。一旦到达send，candidate/call error/HTTP response/timeout/取消/未知都计为provider request并停止transport retry，structured不消费endpoint classifier。每个已返回prepared call恰close一次：send前取消且close成功为零request cancelled failed，send前close失败为零request provider failed；send后取消/deadline/未知或任一close失败立即needs-review并保留reservation，不发布valid、retry或repair。Structured每attempt trusted input bound固定为`max_prompt_utf8_bytes+catalog.input_envelope_token_bound`，联合token/cost按该per-attempt bound与request limit checked乘法预约。Input/output price values与price source ref/version分别成对完整；启用价格必须有完整source，cost关闭允许price values/bounds为null而完整catalog/source identity继续进入route/evidence，任一半pair在claim/provider前以`budget.reservation_rejected`关闭失败且内部`ModelRouteError`不得越过公开seam。Repair limit至少1且所有允许repair用尽才是exhausted；limit0首次失败唯一为invalid/extra。 |
| 核心验证 | Adapter结果进入settlement前必须由核心 validator按同一 canonical schema再校验。核心只消费`Draft202012Validator.iter_errors(value)`直接产生的错误，绝不递归展开`ValidationError.context`：`anyOf|oneOf|not`等直接组合器错误映射为容器path的constraint violation，`allOf`若validator直接产出叶子错误则按叶子keyword/path映射，不能同时追加context子错误。`format`在compiler阶段已拒绝，不创建或依赖`FormatChecker`。Validation path从absolute path按RFC6901 escaping生成；required/extra按schema与instance keys集合展开，不解析message；64项/1024-byte path上限、overflow折叠见OpenSpec usage delta。混合错误首次只要有extra即extra-fields，否则invalid；repair limit至少1并耗尽才是repair-exhausted。允许repair时只在同一冻结deployment/provider/model发起下一request；usage不完整进入unknown/needs-review，不能把已消费调用记零。 |
| Durable evidence | Started/final `ModelUsageEvidence.decision.structured_output`使用`structured-output-evidence-v1` exact keys `schema_version/schema_identity/status/repair_limit/repair_count/provider_request_limit/provider_request_count/replay_identity/validation_issues/error_code`。Issue exact为`{code,path}`，code封闭为`json_invalid|missing_required|type_mismatch|extra_field|value_not_allowed|constraint_violation|schema_invalid|validation_issue_overflow`；path按absolute path/RFC6901生成，required/extra分别追加缺失/额外成员token，列表按`(path,code)`排序去重，超过64项或path UTF-8超1024时唯一折叠为root overflow。混合终态优先级、limit0 invalid/extra和limit>=1 exhausted union由OpenSpec逐值冻结。Started固定count0/null replay/error/空issues；valid固定非null replay、正count、空issues/null error；failed固定非null replay、0..limit request count、null value与既有稳定transport/provider/cancel error；needs-review固定非null replay、`model.provider_side_effect_unknown`，repair/request count各自按耐久事实为精确非负整数或显式null。Text evidence为null，零调用preflight不创建摘要；恢复共同校验response/evidence/schema/replay/attempt/charge，nullable count与已知attempt前缀不得矛盾。 |
| Structured attempt | `StructuredModelAttemptEvidence`保留5.29全部基字段，并要求`structured_output`为`structured-output-attempt-v1` exact object：`schema_version/schema_identity/phase/repair_ordinal/transport_ordinal/prompt_digest/repair_trigger_codes/validation_codes/not_started_proof/cleanup_status`。Initial固定`phase=initial/repair_ordinal=0/repair_trigger_codes=[]`；repair固定`phase=repair/ordinal=1..repair_limit`且trigger codes等于上一轮最后一个已验证、cleanup completed attempt的排序去重非空validation codes；同ordinal所有transport attempts的schema/prompt/trigger codes相同。Validation codes为null、空数组或非空稳定codes。Cleanup status只允许`not_applicable|completed|failed|unknown`：prepare未返回且有发送前proof才可not-applicable；prepared已返回后aclose正常/失败分别completed/failed；无法证明close结果为unknown。Global attempt从1连续并按双ordinal字典序对应。Valid/invalid/extra/exhausted和send后确定failed要求最后cleanup completed；send后cleanup failed/unknown只能needs-review，send前cleanup failed/unknown保留proof/零request并以provider failed停止。Text attempt不得携带structured摘要，structured attempt不得退化为text基类。 |
| Not-started proof | `not_started_proof`为null或`structured-output-not-started-proof-v1` exact object：`schema_version/kind/usage_call_id/operation_identity_digest/route_digest/schema_identity/prompt_digest/attempt/repair_ordinal/transport_ordinal/digest`。Usage/operation/route/prompt/digest为64位小写SHA-256，schema/ordinal与attempt逐值匹配，kind只允许`client_prepare_not_started|cancelled_before_send`。Digest从移除digest后的其余exact字段按`structured-canonical-json-v1`计算，OpenSpec usage delta的695-byte golden preimage固定SHA-256为`940380a8980e64c418b0836366a34520b41e62688610b3e25a288e89940a5d2b`。Proof只由核心协调器在尚未调用prepared send的控制流事实中构造；业务请求、fake脚本、provider原文、adapter或transport classifier无权提交proof。任何已到达send或收到HTTP response的attempt均固定proof=null、计入provider request count并停止structured transport retry；endpoint-bound classifier只服务既有text路径。`side_effect_state=not_started`必须有逐值匹配的核心发送前proof且validation/usage/charge为null或合法零；started/unknown必须proof=null。完整核心发送前proof允许该attempt不计入provider request count；只有mark事务提交状态也确定时才可按actual-zero退款。`DurableMarkStateUnknown`即使保留`cancelled_before_send`零请求proof，也必须以`model.provider_side_effect_unknown`/`needs_review`保存actual=null，并使direct/allocation reservation与owner ledger保持围栏。Proof缺失、篡改、ordinal/prompt/route/schema不匹配一律needs-review。 |
| Stable terminals | `model.structured_schema_unknown`、`model.structured_schema_conflict`、`model.structured_policy_invalid`、`model.structured_invalid`、`model.structured_extra_fields`、`model.structured_capability_unsupported`、`model.structured_route_not_allowed`、`model.structured_repair_exhausted`、`model.structured_replay_conflict`、`model.input_too_large`、`model.provider_failed`、`model.provider_retry_exhausted`、`model.invocation_cancelled`、`model.provider_side_effect_unknown`、`budget.reservation_rejected`。未知schema、identity冲突、repair-policy越权分别使用前三个错误；deployment capability或provider protocol缺失都由structured协调器统一映射为`model.structured_capability_unsupported`，底层`model.capability_unsupported`不得逸出。只有`valid`是业务成功；已建立claim后的确定失败使用`failed`，不能证明结果/usage才提升needs-review，禁止自动重发。 |
| 兼容与非目标 | 不改变`ModelResponse.output_text`、text-only/non-stream、普通文本stream或Phase18.2 route-chain顺序；不做structured streaming、跨provider structured fallback、tool call/工具循环、工具执行、fake隐式后备或Phase21重构。示例只能在公共seam稳定后迁移且不能反向定义核心DTO；Phase19只迁移严格Registry加载所必需的Dev Assistant宽松工具结果和RAG Assistant宽松组裁字典。RAG SHALL 使用两个互斥exact object的封闭联合：`status=no_source`时`assembly_truncation`只允许`{}`，且`assembly_id/model_provider=null`、citations/source refs为空，由未创建Context Assembly的本地无检索结果分支构造；`status=completed`时只允许含`input_count/retained_count/truncated_count/dropped_count/used_tokens/fragment_count`六个必填非bool非负整数的`RagAssemblyTruncation`，并要求非空`assembly_id/model_provider`，仅映射既有Context Assembly producer。部分六字段、状态与变体混搭或额外字段均fail closed；不伪造六个零计数或assembly耐久记录。不迁移含调用后trace字段的ticket triage最终输出，不创建第二套schema控制面，也不改写Context Assembly耐久schema。 |
| 验证 | 至少两个不同provider id的显式doubles/fakes从bound公共seam覆盖success、missing/type/extra、unknown schema、unsupported、多候选、budget、repair success/exhaustion、exact/conflict replay、durable recovery与unknown/needs-review；聚焦contract/integration/eval和既有text/stream/route-chain回归均须通过。默认零真实provider请求，未授权live项报告`hosted-unverified`而不是PASS。 |

### MOD-006 受控模型工具循环

本节冻结 REQ-029 的 module/runtime/event 合同，不新增 HTTP route，也不改变 `TLS-001`～`TLS-003` 人工 CLI/runtime 工具入口。受控模型工具循环的默认验证只使用 fake model/tool doubles；真实模型与真实外部工具联合调用需要独立授权。

| 契约面 | 冻结语义 |
|---|---|
| 单轮结果 | 模型公共结果使用 provider-neutral 判别联合 `ModelTurnResult`，`kind` 只允许 `final_text|final_structured|tool_intent`，三种 payload 互斥；`final_text` 复用现有 `ModelResponse`，`final_structured` 复用 MOD-005 结果，`tool_intent` 使用本节 DTO。Capability 进一步收窄合法分支：`tool_intent` protocol 只返回 `final_text|tool_intent`，`structured_output` 只返回 `final_structured`，既有 text protocol 只返回 `final_text`；不得根据任意 JSON 字段、字符串内容或 SDK event 类型猜测判别值或跨 capability 冒充结果。 |
| Adapter candidate | Adapter 只可返回 `ProviderToolIntentCandidate`：`schema_version=provider-tool-intent-candidate-v1`、provider/model、tool name、JSON object arguments、被提供的 tool schema identity 和恰好一组本次 model attempt/usage 事实。SDK tool-call、callable、handler、client、raw response 和 credential 不得越界。Adapter 不注册 executable callback，不执行 provider-native tool runtime；tool-call streaming/args delta 本阶段非目标。 |
| Tool catalog / selection | Agent 配置继续只使用 `tool_allowlist`，Registry 必须把它逐值同序投影为 public descriptor `tool_policy.allowed_tools`，不新增顶层 `allowed_tools` 配置字段。Runtime 从该绑定 descriptor 投影与 `ToolRegistry` 只读 descriptor 构造有序 `tool-catalog-v1`，每项包含 tool name、input schema ref/version/digest、action/resource 和 catalog ordinal；catalog digest 进入 route/turn identity。公开 exact `ToolCatalogSelection` 唯一字段为 `tool_names: tuple[str, ...]`，只作为 bound tool-intent seam 的独立关键字参数，不扩展 `ModelRequest`：参数缺省表示完整绑定 catalog，显式空 tuple 表示本轮空 catalog，非空 tuple 必须为 descriptor 列表的唯一保序子序列。未知、重复、重排、额外字段、非 string 值或塞入 `ModelRequest` 均以 `model.tool_catalog_conflict` 在 claim/reservation/client/provider 前零调用拒绝。Config reload 只影响新 loop。 |
| Tool-enabled request | 真实 `tool_intent` deployment 必须只声明该 capability，固定 legacy 单 route、`max_attempts=1`、空 fallback model/route 与 response classifier、`max_structured_repair_attempts=0`；不得与 text/stream/structured capability 共用一个 deployment。其唯一请求形态为 `request_shape_ref=single-user-text-with-tool-catalog`、`request_shape_version=v1`。核心从选定 `tool-catalog-v1` 生成 `provider-tool-catalog-v1` exact object：顶层只含 `schema_version` 与有序 `tools`；每个 tool exact 包含 `name/input_schema_ref/input_schema_version/input_schema_digest/input_schema/ordinal`。`input_schema` 必须逐值等于 Registry strict canonical JSON Schema，catalog bytes 固定使用 UTF-8、`ensure_ascii=false`、排序键、紧凑 separators、保留 JSON null、拒绝 NaN/Infinity/非 JSON 值；adapter 只能把该核心快照映射为 vendor tool definition，不得增删、重排或从 SDK/current Registry 补值。既有 `single-user-text-no-tools/v1` 携带非空工具定义必须在 provider 前 `model.tool_catalog_conflict`。 |
| Tool request budget | Tool-enabled model catalog entry 必须以 `model-catalog/v2` 冻结非 bool 非负 `max_tool_catalog_utf8_bytes`；no-tools v1 entry 不得补该值。选定 provider catalog 的实际 canonical UTF-8 bytes 必须不超过该上限；`trusted_input_token_bound=len(prompt UTF-8 bytes)+len(provider tool catalog canonical UTF-8 bytes)+input_envelope_token_bound`，并沿用 checked `utf8-bytes-plus-envelope/v1` 计算 per-attempt token/cost 与调用级 reservation。静态 deployment ceiling 使用冻结 `max_prompt_utf8_bytes+max_tool_catalog_utf8_bytes+input_envelope_token_bound+max_output_tokens`，动态 reservation 使用实际 catalog bytes与请求级`output_token_cap`。Cost enabled时静态输出项为`max_output_tokens*output_price`，动态输出项为`output_token_cap*output_price`。Request shape、model catalog ref/version/digest、tool catalog digest/bytes/max bytes、可信输入上界与 output cap 必须逐值进入 route、snapshot、operation identity、`model.invoke` approval arguments/continuation、started/final usage evidence 和 exact replay；恢复不得从 current Registry 重建。Shape 不匹配、catalog 超长、schema bytes/digest 漂移、checked arithmetic 溢出或预算不足均在 reservation/client/provider 前零调用拒绝。 |
| Tool request identity / snapshot | `tool-intent-request-identity-v1` exact fields 固定为 `schema_version/request_shape_ref/request_shape_version/model_catalog_digest/tool_catalog_digest/tool_catalog_utf8_bytes/max_tool_catalog_utf8_bytes/trusted_input_token_bound/output_token_cap`，整数拒绝 bool，摘要为 64 位小写 SHA-256。该 exact object 使用与 provider catalog 相同 canonical JSON 规则并计算 `tool_request_identity_digest`。既有私有 route/budget snapshot 保存完整 `provider-tool-catalog-v1` canonical bytes 与该 identity，公开 route/evidence 只投影 identity/digest，不保存 prompt 或 schema definition；恢复只读私有 snapshot，不能从 current Registry 重建。`tool-intent-policy-approval-arguments-v1` 瞬时 exact preimage 固定为 `schema_version/usage_call_id/operation_identity_digest/request_arguments_hash/tool_request_identity_digest`，continuation/checkpoint/grant 保存并交叉校验后两个 digest；批准只绕过同一 soft policy，仍重检当前 hard balance。缺失、额外、错型或任一字段漂移在 provider 前关闭失败。 |
| `ToolIntent` | 核心验证 candidate 后生成 exact DTO：`schema_version=tool-intent-v1`、`loop_id`、非 bool 正整数 `turn_ordinal`、`tool_call_id`、`tool_name`、JSON object `arguments`、`arguments_digest`、`tool_schema_ref/version/digest`、`model_usage_call_id` 和 `catalog_digest`。Loop、turn、call、usage 身份只由受信 tenant/run/agent/request/trace/operation key 与 ordinal 派生，provider、Agent input 和 HTTP/CLI 调用方不得自报。 |
| Registry resolve | `ToolRegistry` 增加无工具副作用的只读 resolve seam，只进行 tool name、Agent allowlist、catalog/schema/source identity 与 arguments validation，返回不含 handler/callable 的 `ResolvedToolIntent`。未知、未授权、schema 无效或身份漂移在 policy、execution claim 和 handler 前以稳定错误关闭；允许写脱敏 validation/audit evidence。既有 `call`/`call_approved` 在执行前重新验证同一绑定，防止 TOCTOU。 |
| Agent 循环配置 | Agent 任一有效 route 支持 `tool_intent` 时，exact `model_tool_loop` 必须存在且只含全部必填的 `max_turns`（非bool整数1..64）、`max_total_tokens`（非bool正整数且不超过根token预算）、`max_total_cost_usd`（null或有限非负；根cost非null时不得为null或超过根值）、`max_tool_output_bytes`（非bool整数1..1048576）、`max_duration_seconds`（非bool整数1..3600）；无该 capability 时对象必须缺失。不存在默认值、环境或deployment回退。Registry 在executor/client导入前校验并投影为不含动态余额、deadline、secret或本地路径的只读descriptor summary；任一非法 sibling 使全量加载失败。 |
| 绑定循环入口 | Runtime 通过 `build_execution_context()` 暴露 exact `BoundModelToolLoopService.run(request: ModelRequest, *, operation_key: str, tool_selection: ToolCatalogSelection | None = None, limits: ModelToolLoopLimitOverrides | None = None)`；业务 executor 不能提交 loop/tool/usage identity、ToolRegistry handler、policy result、approval grant 内容或动态余额。`ModelToolLoopLimitOverrides` 是独立 exact DTO，只含五个全部必填 nullable 同名字段；DTO缺省或字段null表示继承Agent maximum，非null只能逐项缩小并复用配置的类型/范围规则。缺失/额外字段、bool、NaN/Infinity、负值、扩大值、任意dict或caller自报started_at/deadline均在claim/reservation/client/provider/tool副作用前拒绝。循环内部每个 model turn 继续通过既有绑定 model invocation/shared-budget seam。 |
| 固定顺序 | 每轮唯一顺序为 `model turn → ToolIntent validation → Registry resolve → policy/audit → optional approval/checkpoint → durable tool claim → tool execution → output guard → ContextAssembler → next model turn`。Tool-intent protocol 只有 `final_text` 能成功结束 loop；`final_structured` 是 cross-capability protocol violation，必须结算已发生model impact后关闭失败，结构化业务结果只走独立 MOD-005 seam。Deny、无效 intent、上限耗尽或确定失败按稳定终态收口；任一路径不得跳过 Registry、policy/HITL、output guard、ContextAssembler、usage 或 evidence。 |
| Policy/HITL | `deny` 不创建 tool execution claim且 handler 为零调用；`require_approval` 只创建与原 loop/turn/tool call、arguments/schema/action/resource、tenant/identity/run/agent、request/trace 和 catalog digest逐值绑定的 approval/checkpoint。批准恢复必须持有 active grant/lease，重算全部 identity 与 current hard bounds，并通过 `ToolRegistry.call_approved` 的唯一 claim seam执行一次；缺失、过期、跨 run、字段漂移或同步篡改均在 handler 前 fail closed。 |
| Tool result 回注 | `ToolCallResult` 先经过现有 output guard；大 payload/secret 只留下脱敏摘要和 artifact ref。进入 ContextAssembler 的 `ContextFragment` 固定 `trust_level=untrusted` 并保留 source ref、artifact ref、token estimate、truncation 与 injection summary；assembled text 只能作为引用内容，不能覆盖 system/developer/policy。下一轮 model request 绑定 Context Assembly output ref/digest，不重新读取未冻结原始结果。 |
| 全局边界 | Loop 只从绑定 Agent descriptor 的 `model_tool_loop` summary 取得 maxima，应用可选 overrides 后，以受信 `loop_started_at + effective max_duration_seconds` 推导并冻结 `deadline_at`；调用方不能自报绝对时间。Freeze 包含五项 effective bounds、deadline 和 tool catalog identity。每个 model turn 使用同一 root execution-tree ledger 的独立稳定 usage call，所有 reservation/actual 累计受冻结上限约束。Approval、resume、reload 和 restart 不能重置 ordinal、deadline 或余额，也不能从当前配置重建。 |
| 耐久状态 | `model-tool-loop-state-v1` 至少保存 loop identity/request fingerprint/catalog digest、冻结上界、status、next turn ordinal、累计 usage、current model usage call、tool call/approval/checkpoint/context refs、最终 result/error 和 evidence version。状态只允许 `active|waiting_approval|completed|failed|cancelled|needs_review`；字段组合和 transition 使用封闭联合，未知字段或非法倒退 fail closed。`durable-tool-loop-resume` capability 强制以 `0018_model_tool_loop_state` 从0017新增loop协调表、单行`model_tool_loop_schema_marker`、tool/context nullable v1 identity及tool execution lease/fence/proof字段。Marker exact key固定`model-tool-loop-v1`且`evidence_seen`初始false；任何首条v1 evidence在同一UoW先把它单调置true，应用/维护入口不得清零或删除。SQLite/PostgreSQL 使用同一 repository/UoW contract并验证upgrade、legacy兼容、旧binary拒绝和downgrade。Marker为true或扫描存在v1 evidence时downgrade永久拒绝，删除、置空或导出业务证据不得绕过。 |
| At-most-once 与 replay | 每个普通或 approved 工具调用都在 handler 前以 `tool_call_id` 建立唯一 durable execution claim。新模型驱动claim状态只允许 `claimed|executing|completed|failed|needs_review`，并保存execution lease digest、正整数fence、lease expiry、nullable handler-started时间和not-started proof。首次claim进入`claimed`；existing claimed只有在旧lease过期后，owner UoW以CAS同时轮换lease、递增fence并保存exact `tool-handler-not-started-v1`（tool call/binding、prior/next fence、旧expiry、固定reason与canonical digest）才可继续。只有matching active lease/fence确认提交`claimed→executing`后才铸造一次性`ToolExecutionPermit`并调用handler；旧owner在handler边界前失败。Completed replay返回既有result ref，确定failed replay返回既有失败；executing、该transition提交确认未知、handler可能已开始、结果/commit未知一律进入needs-review，不得降回claimed或自动重放。每个model turn复用既有usage claim/attempt evidence；exact replay不再次调用provider，identity conflict零副作用。Context Assembly按input/output identity exact replay，不从当前tool payload重组旧轮次。 |
| CanonicalEvent | 受控模型工具循环生产既有目录中的 `tool.call.started|completed|failed`、`context.assembly.started|completed`、`approval.required|resolved` 及每轮既有 model usage evidence，不新增等价事件类型。稳定 event id 绑定 loop/turn/tool call；operation capacity 和 outbox 在对应外部副作用前预约。`tool.call.started` 只有 durable claim 与 handler 即将执行时出现；无效 intent、Registry/policy deny不伪造 started。所有 prerequisite evidence 完成前 run terminal 被围栏。 |
| Unknown 与 terminal | Provider、tool、result persistence、Context Assembly、event publication 或提交确认无法证明确定结果时，loop 与相关 usage/tool claim/owner ledger 进入 needs-review，保留预算、event capacity 与 lease 围栏；不得自动 retry、切 provider、再次调用 handler、发布 completed/final usage/run terminal或把 token/cost/tool impact记为零。Recovery 只补投已耐久 exact evidence或返回既有结果。 |
| 稳定错误 | 新增 `model.tool_intent_invalid`、`model.tool_catalog_conflict`、`model.tool_loop_limit_exceeded`、`model.tool_loop_replay_conflict`、`model.tool_loop_needs_review`；Registry/执行继续使用 5.21 的 `tool.*` code，policy/approval/预算/event 继续使用既有稳定 code。公开错误不包含 arguments、tool output、prompt、secret、SDK exception、本地路径或动态余额。 |
| 兼容与非目标 | 现有 text completion、text streaming、route-chain、MOD-005 structured output、人工 tools CLI/runtime、approval API 与 fake eval 保持兼容。本 change 不做 structured streaming、tool-call streaming、跨 provider structured fallback、动态 marketplace、后台 scheduler、真实 provider+真实工具默认 smoke 或 typed-service/state-kernel 架构重构。 |
| 验证 | `provider-neutral-tool-intent` 从两个不同 provider id doubles 验证 final/tool判别、SDK隔离、tool selection缺省/空/保序子集及非法选择零副作用、tool-enabled request canonical bytes/golden vector、shape mismatch、超长 schema/catalog、预算不足、恢复 catalog 漂移、既有`model.invoke` Policy/HITL waiting与approved continuation、provider至多一次和零工具执行；`policy-gated-tool-loop` 从公开 bound loop seam 验证Agent `model_tool_loop` exact/required-iff/根预算/descriptor、override继承与缩小、受信deadline、`final_structured`拒绝、allow/deny/工具approval、result guard/context和各上限；`durable-tool-loop-resume` 在 SQLite/真实 PostgreSQL 覆盖每个崩溃窗口、exact/conflict replay、unknown、event/terminal fencing。默认全部网络与真实工具调用计数为零。 |
...
| 冲突 | direct 与 `_FILE` path 在大小写不敏感 canonical 化后比较；同时设置同一 canonical path、多个原始别名映射同一路径、空 segment 或非法 path 必须在读取 secret 内容和 merge 前结构化失败，不静默选择一个。deployment/credential YAML key 必须匹配 `[a-z][a-z0-9]*(?:_[a-z0-9]+)*` 且最长 64；env path 可使用常规大写，但规范化后不得碰撞。 |
...
| model / embedding adapter | `MOD-001` evidence seam；真实非流式 model 另受 `MOD-002` execution seam 约束 | 所有 adapter 必须输出 provider-neutral 最终 `ModelUsageEvidence`，并通过 EventBus/TelemetryFacade 关联 run/trace；业务 agent 不拼 raw usage。真实 model adapter 只接受 composition 冻结的 plan/permit，不读取 request endpoint/credential。受控增量文本流契约另行定义 provider-neutral 文本 delta、部分 usage/unknown 与有界 event-capacity，不由本 execution seam 隐式扩张。 |
| service-app API run execution | `RUN-001` → runtime orchestrator → `MOD-002` module seam | FastAPI route 不直接构造 SDK client；app composition root 在接受 run 前全有或全无地构造 router/provider-neutral lazy factories，但不预构造真实 client。合法调用通过动态 hard eligibility、reservation 与 permit 后才获取 client lease；lifespan 退出时通过 `RuntimeComponents.close()` 先幂等关闭 factory/已构造 leases，再关闭 queue/storage。无新增真实模型 HTTP route。 |
| service runtime worker | queue/DBOS execution → `MOD-002` module seam | Worker 从同一 typed settings、registry 与 frozen snapshot 构造相同 model invocation/lazy factory seam；queued replay 先恢复 settlement，不重放未知 provider 副作用。worker shutdown 通过统一 runtime components 关闭 factory 与已构造 async client leases。 |
| `agent-harness run` 与内部 CLI model execution | `CLI-RUN-001` → `MOD-002` module seam | CLI 不让 provider ambient env 形成出站配置/身份路径、不直接持有 SDK object；命令作用域由 composition 创建 provider-neutral lazy factories，并在成功、失败或取消路径幂等 `aclose()` 后退出。 |
...
- 当前两个 stream 入口仍是 transport-only reader；`controlled-model-streaming` 提供 provider-neutral `model.output.delta/completed` producer并复用这些 committed reader。持久化顺序、容量、安全与取消契约以 5.9.1 为准；不新增 provider cursor 或第二套 stream endpoint。
...
- [x] MOD-004 真实模型普通文本增量 producer 已实现；5.9.1 冻结 delta payload/identity、跨 chunk 输出安全、event capacity、completed/usage/terminal 顺序、取消/unknown 与 provider 不重放契约。真实 stream 因隔离 credential 前置不完整保持零调用 `hosted-unverified`，不作为真实 provider 成功 PASS；RUN-006 transport 的既有验收保持独立。
