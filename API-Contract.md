...
| `attempts[]` | `attempt`、`outcome`、`side_effect_state`、`completion_observed`、`http_status`、`retry_after_ms`、`input_tokens`、`output_tokens`、`cost_usd`、`cost_status`、`budget_charge_tokens`、`budget_charge_cost_usd`、`latency_ms`、`error_code` | `outcome=completed|retryable_status|failed|cancelled|unknown`；`side_effect_state=not_started|started|unknown`。未知字段为 null，不用零冒充。SDK exception、raw body、header、prompt、response id 和完整 URL 不得进入该结构。 |
...
| 字段 | 约束 |
|---|---|
| 状态 | 协议增量及对应 runtime 当前保存在 active OpenSpec change `controlled-multi-provider-failover` 的实现工作树中，尚未 sync/archive。Typed route refs、冻结 chain/state、bound completion/streaming、审批续跑、shared-budget/migration、evidence validator 与 live/CI producer 已接线；最终全量门禁和 fresh `1+2` 实现审查完成前不得解释为 `ready-to-archive`。 |
