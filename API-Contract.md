...
该目录是合法 `CanonicalEvent` 类型词汇，不是 producer 完成清单。既有一次性模型调用只形成 request-started 与最终 usage evidence；`controlled-model-streaming` 当前实现候选已按 5.9.1 生产 `model.output.delta` / `model.output.completed`，但在最终 `1+2` 审查通过前不得描述为已归档能力。RUN-006 / CLI-EVT-001 只传输已经持久化的事件；只有 provider → router → invocation → durable event 生产链与对应审查证据同时成立，才可宣称 provider 文本增量 producer 就绪。
...
- `controlled-model-streaming` 的 producer 契约固定在 5.9.1；当前实现候选不得改变受信 stream operation kind、最大 delta 数、分片/合并策略、稳定 event id、可见性、安全、顺序或 unknown 规则后再反向修改契约。SDK token 数量或 HTTP/Agent 自报值不得作为容量上限；不能在无界 provider stream 已开始后才发现 CanonicalEvent 容量不足。
...
本节是 `controlled-model-streaming` 的冻结 API/event contract；当前生产链已形成实现候选，仍等待最终 `1+2` 审查。它只增加 provider-neutral 普通文本 producer，不新增 HTTP/WebSocket route，不透传 SDK event、provider cursor、reasoning/tool/structured delta，也不改变 MOD-002 一次性 `complete` seam。默认 provider 仍为 fake；真实流式验证必须同时取得既有真实 provider opt-in 与独立 stream opt-in，默认 CI 不发起网络请求。
...
### MOD-003 受控跨 deployment/provider fallback（Phase 18.2 规划）

| 字段 | 约束 |
|---|---|
| 状态 | 仅规划，尚未创建 OpenSpec change、修改 runtime 或声明实现完成。Phase 18.2 `controlled-multi-provider-failover` 必须在 Phase 18.1 验证、review、同步并归档后另行获得用户授权。 |
| 公开入口 | 不新增 HTTP endpoint。API、worker 与 CLI 继续通过既有 run/model invocation seam；Phase 18.2 只扩展 typed Agent/deployment model policy、provider-neutral route plan/evidence 和 composition。 |
| 配置 | Agent policy 以有序 route refs 表达 `fallback_routes`，每个候选稳定身份为 `(deployment_id, model_id)`。`provider_kind`、endpoint、credential ref、model catalog、capability、timeout/retry、Bulkhead 与价格只从候选 deployment 的 typed config 派生；同一 provider kind 的不同 vendor endpoint/credential 仍视为不同 deployment。 |
| 请求 | Request 只能从 deployment∩Agent 已冻结 route chain 中选择或删减候选，不得新增或重排 deployment/provider/model，不得声明 endpoint、credential 或 SDK adapter。非法字段继续使用 `model.request_invalid`，越权/空交集继续使用 `model.route_not_allowed`。 |
| Route plan | 首次 client、DNS、HTTP 或预算副作用前冻结完整 route chain、chain identity、候选 ordinal，以及各候选 endpoint/catalog/credential identity、capability、deadline/retry/Bulkhead 与 token/cost 上界。Public DTO/evidence 只包含去敏 identity，不包含完整 URL、secret、header 或 SDK 对象。 |
| Failover 条件 | 静态 hard eligibility 可以在零 provider 副作用时跳过候选。运行时只在 client/send 前失败，或受信、版本化 classifier 明确证明当前候选 `not_started` 时进入下一候选；普通异常文本、HTTP body、缺失/冲突 classifier、read timeout、取消、response identity、usage、文本或 delta 均不得授权 failover。 |
| Budget / settlement | 每个候选按自己的冻结 catalog/envelope/price/attempt policy 计算 reservation。进入下一候选前，上一候选必须已耐久收敛为 not-started并安全释放/转移 reservation，下一 reservation 原子建立；unknown 保留原 reservation/needs-review。Replay 只恢复同一 chain/ordinal/settlement，不再次调用已开始 provider。 |
| Streaming | 首个 delta 尚未观察且当前候选可证明 not-started 时才允许按冻结链切换；观察或提交任一 delta 后，取消、deadline、SSE/CLI 断线、Last-Event-ID、恢复和重连都不得触发跨 provider fallback。 |
| Evidence | 在 5.29 provider-neutral evidence 基础上新增或版本化 route-chain identity、candidate ordinal、去敏 failover reason 与 reservation transition；每个实际 attempt 仍逐项记录 deployment/provider/model、side-effect state、usage/cost/latency。字段级 schema 必须由 Phase 18.2 OpenSpec/API delta 在实现前冻结，不用本规划伪造当前已存在字段。 |
| 错误 | 已开始或 unknown 候选沿用 `model.provider_side_effect_unknown`/原稳定 provider error并停止。所有候选均以安全 not-started 或静态拒绝耗尽时，Phase 18.2 change 必须冻结一个 provider-neutral exhausted error及封闭 cause summary；不得把 raw provider error、endpoint 或 credential 放入公开 envelope。 |
| Fake / 离线 | Fake 只能作为 profile/Agent 显式 route，不能是全部真实候选失败后的隐式尾项。默认 quality/test/eval/smoke-local 继续零网络；多 provider live smoke 必须使用两个隔离 credential refs 和两个受信 deployment，并继续受用户单独授权、opt-in 与去敏证据门禁。 |
...
- 当前两个 stream 入口仍是 transport-only reader，但 `controlled-model-streaming` 已形成 provider-neutral `model.output.delta/completed` producer 实现候选并复用这些 committed reader；该候选尚未通过最终实现审查，也未 sync/archive。持久化顺序、容量、安全与取消契约以 5.9.1 为准；不新增 provider cursor 或第二套 stream endpoint。
...
- [ ] MOD-004 真实模型普通文本增量 producer 已形成实现候选，但最终实现审查尚未通过，未 sync/archive；5.9.1 已冻结 delta payload/identity、跨 chunk 输出安全、event capacity、completed/usage/terminal 顺序、取消/unknown 与 provider 不重放契约。候选已用 public-seam 红测和局部 event/SQLite/真实 PostgreSQL/streaming/SSE/CLI tests 验证，后续必须先闭合审查 finding，不能反向改契约迎合代码。该未完成审查项不否定 RUN-006 transport 的既有验收。
