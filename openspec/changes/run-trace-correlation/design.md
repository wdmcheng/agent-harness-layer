...
先完成 `embedding-cache-tenant-isolation` 的插入式 revision `0012a_embedding_cache_tenant_scope`，再增加 trace normalizer 与失败合同，并以 Alembic revision `0013`（`down_revision="0012a_embedding_cache_tenant_scope"`）增加关系数据 migration/backfill、local JSONL 离线 upgrader 和 repository 非空门禁；随后切换 API/CLI/runtime/worker/approval/event composition，最后运行 local/service、restart/resume 与 model usage 前置合同。回滚 application code 前必须证明没有新格式记录依赖非空 trace。`0013` downgrade 同时要求 evidence 全空和 Alembic x 参数精确为 `allow_empty_evidence_downgrade=true`；参数缺失、重复、值非法或存在 binding、任一 run-scoped canonical trace/backfill evidence 都必须在 DDL 前拒绝，保留兼容读取且不删除 evidence。`0012a` 自身的降级仍由 embedding cache evidence 门禁独立控制。local JSONL 只通过 journal/backup 恢复，不覆盖成功后的新 evidence。完成后停在 `ready-to-archive`，不自动归档。
