...
2. **受信 root 与文件验证在打开前后都 fail-closed。** 生产默认 root 是 `/run/secrets`，测试可以显式注入临时 root。输入必须是绝对路径；规范化父路径和目标都必须位于 root 内，目标不得是 symlink、目录或其他特殊文件。打开时采用避免跟随 symlink 的受控方式，并在读取后复核文件 identity，降低检查与读取之间替换风险。无法满足平台安全条件时拒绝而不是回退。
...
6. **部署装配只挂载引用。** Compose 为 API、worker 和 migration 使用同一只读 secret mount 与 `_FILE` env；`.env.example` 只给路径和生成说明，真实文件由调用方在隔离环境创建并清理。PostgreSQL 镜像自身的 credential 行为不在本 change 重构范围，但应用进程不得再依赖仓库中的明文 secret。
...
- wheel-only 复制模板与 `make smoke-service` 验证同一只读 mount 被 API/worker/migration 消费。
- 使用唯一 secret fixture 扫描 stdout/stderr、error envelope、doctor、health、日志、trace、eval、audit 与 event evidence，断言原值不存在。

## Risks / Trade-offs

- [Risk] 路径预检与读取之间发生替换 → 使用非跟随打开、普通文件/identity 复核；平台不支持时拒绝。
