...
- 为已有 nullable trace 数据提供确定性、幂等的 migration/backfill；跨租户 parent edge、孤立、成环或冲突 lineage 在 DDL/UPDATE 前整批拒绝。失败事务可回滚、中断 bundle 可恢复；成功迁移后 downgrade 同时要求 trace evidence 全空和显式 Alembic `-x allow_empty_evidence_downgrade=true`，不得删除或覆盖新 evidence。读取兼容只允许存在于迁移窗口，新的 run-scoped record 不得继续写 null；合法 non-run evidence 仍保留独立 nullable trace 语义。
- 保留已发布 `0013_run_trace_correlation` 的 revision 身份，以线性后继 `0013a_run_trace_event_hardening` 收敛早期同名 `0013` 与当前最终事件 shape 的漂移；普通入口只认 `0013a` head，旧 stamp 不得仅凭 version 字符串假通过。
...
- `storage-migration-uow`：固定 `0013` 从 `0012a_embedding_cache_tenant_scope` 继续的 trace upgrade/backfill，并由 `0013a_run_trace_event_hardening` 对旧/最终两种完整 `0013` shape 做安全前滚，以及 evidence-aware downgrade 的 SQLite/PostgreSQL 一致性。
...
- 受影响数据：现有 nullable run execution context、approval、event、audit/trace refs 需要确定性 backfill；不得删除历史 evidence。`0013` 以已闭环 embedding cache 租户隔离的 `0012a` 为直接前置，`0013a` 是事件 shape 硬化后的唯一 head。
