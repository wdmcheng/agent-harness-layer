...
4. **插入式 revision 保留已审迁移编号。** 新 revision 为 `0012a_embedding_cache_tenant_scope`，`down_revision="0012_service_runtime_execution_context"`；trace `0013` 明确从 `0012a` 继续。替代方案是把 trace/usage/delegation 整体改为 `0014/0015/0016`；拒绝，因为这只增加契约 churn，不提供额外安全性。
