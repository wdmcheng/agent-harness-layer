...
13. **Usage operation stable key 与 immutable identity 分离。** Direct stable key固定为`(tenant_id,budget_owner_run_id,usage_call_id)`；delegation child allocation stable key固定为`(tenant_id,budget_owner_run_id,delegation_claim_id,usage_call_id)`；key只负责找到唯一operation。两者以同一`identity_schema_version`和canonical JSON hash证明是不是同一调用。Identity在final actual route/trusted intent确定后、reservation前生成，封闭包含`ownership_kind=direct|allocation`、`run_id`、`agent_id`、allocation时非空的`delegation_claim_id`、`usage_kind`、稳定语义operation slot、tenant-scoped keyed request fingerprint及key version、root snapshot ID、适用agent sub-snapshot ID、provider/model、price source ref/version、cache key digest、cost-enabled状态与token/cost trusted bound。Canonical JSON使用UTF-8、排序键、紧凑分隔符并拒绝非有限数值；只持久化hash、key version和上述非敏感refs/bounds，不保存prompt、embedding原文或fingerprint key。动态余额、event capacity、approval result、cache hit/miss结果和provider result不得进入identity。Cache lookup的稳定cache-key digest进入identity，但hit/miss是首次durable result：提交前可重查，提交后同identity重放首次结果。相同stable key只有identity逐值/hash相同才exact replay；任一字段不同都在owner/relation、子额度、parent余额与capacity检查前返回内部`budget.operation_conflict`且外部副作用为零。Allocation冲突若由parent delegation seam对外收口，必须映射既有`delegation.execution_failed`，不得扩张公开delegation错误集或泄露identity。Unique race回滚后按同一规则重读。
...
- Repository/UoW：同 tenant 独立 root owner 隔离、同 root direct/delegation/allocation 合流、direct/allocation immutable identity exact replay/conflict、direct→direct、direct→delegation、delegation→direct、混合真并发与 token/cost 双维竞争。
...
对可backfill tree，migration规范化非空owner：root取自身`run_id`；child由tenant-fenced `agent_runs.parent_run_id`与唯一`agent_delegations.child_run_id`共同证明同一root，并取该root为`budget_owner_run_id`。随后只从immutable registry/config/catalog、root source与允许target各自的descriptor/model-policy/route/price versions逐值校验hash、tenant/agent、hard limits、cost-disabled状态与tree snapshot继承；source/target descriptor不同不构成冲突，child必须命中同一owner snapshot中自己的target sub-snapshot。Current resolver/current config、reservation、actual、默认值或零值不得补snapshot。对legacy direct或child usage还必须存在可信的ownership kind、stable operation slot、keyed request fingerprint/key version、实际route与trusted bound，才能分别回填direct/allocation immutable identity；child还必须有唯一delegation claim关联，缺一项即不得把历史row猜成exact-replay-capable operation。Snapshot与identity通过后，root自身usage才建立direct claim，child usage只建立allocation；settled/unknown与`0015`四态按固定矩阵迁移。DDL完成后部署`0016` writer：root创建、tree snapshot与ledger原子提交；direct/delegation新claim分别与`0014`及`0014+0015`记录同UoW提交。最后恢复workers。Downgrade只允许shared-budget evidence全空且精确opt-in；有evidence时不删除或释放事实。本 change 只推进到`ready-to-archive`，不提前sync/archive，不发布。
