...
- Direct claim 将 `(tenant_id,budget_owner_run_id,usage_call_id)` 作为 stable key；delegation child allocation 将 `(tenant_id,budget_owner_run_id,delegation_claim_id,usage_call_id)` 作为 stable key；二者都另外持久化相同版本化 immutable operation identity。Identity 在最终实际 route 与 trusted intent 确定后、任何 reservation 或外部副作用前生成，覆盖 ownership kind、run/agent/usage kind、稳定语义调用槽位、tenant-scoped keyed request fingerprint、root snapshot 与 target sub-snapshot、provider/model/price version、cache key digest和各启用维度 trusted bound；排除 prompt/embedding 原文、动态余额、event capacity、审批结果和 provider result。同 key 同 identity 才可 replay，同 key 异 identity 固定内部 `budget.operation_conflict`；allocation 冲突若向 parent delegation 传播，外部仍封闭为 `delegation.execution_failed`。
...
- 数据：新增 `0016` shared parent ledger、operation claim 与 delegation child allocation，以非空 `budget_owner_run_id` 关联 execution-tree root，并关联既有 `usage_call_id` 和 delegation reservation；ledger 冻结 tree catalog，direct/allocation detail 保存版本化 immutable identity hash及其非敏感关联字段。
