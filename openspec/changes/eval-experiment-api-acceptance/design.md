...
2. **Create 幂等使用 `(tenant_id, idempotency_key)` 唯一约束 + canonical request hash。** 同 hash 返回原记录，不同 hash 映射稳定 409；repository 在同一 transaction 内判定，避免并发双写。仅用进程内 cache 无法跨 worker/restart。
...
- Persistence：并发/重启后的 create hash、accept unique record/audit，SQLite 与 PostgreSQL contract。
...
本变更不新增 revision，部署前必须已有上游 `0009` schema 和 ExperimentService。新 routes/CLI 可与旧基础 eval endpoints 并存；代码回滚时保留 experiment/acceptance/audit 数据，新版本可再次读取。不得通过 downgrade 删除非空 acceptance evidence。
