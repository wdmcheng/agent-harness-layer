...
2. **Regression 先预留，optimization/holdout 再做确定性多标签分层。** `regression_policy` 可引用固定 case ids 或 `regression=true` metadata；余下 cases 以 tenant/agent/dataset/case id 的 SHA-256 作为稳定 tie-break，按每个标签的 holdout quota 贪心分配。备选纯随机 split 会漂移，纯全局 hash 无法在小数据集保留标签分布。
