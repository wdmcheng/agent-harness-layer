...
9. **RUN-002 与 aggregation 原子切换。** 同一 change 新增 `RunDetailResponse` 的 agent/parent/delegation 字段、route 映射和 drift test；提前声明空字段或另增 delegation HTTP route都会制造不真实 API，因此拒绝。
