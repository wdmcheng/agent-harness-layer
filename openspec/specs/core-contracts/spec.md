...
## Purpose
定义 `agent_harness` 公共边界 payload 的稳定契约，包括 DTO JSON 序列化、typed error envelope、trust/source/context refs、guardrail decision，以及核心契约与 vendor SDK 的隔离要求。该 spec 是后续 runtime、API、tools、retrieval、observability 与 audit event 复用的基础。
## Requirements
