# vendor-boundary-doctor Specification

## Purpose
定义 vendor SDK import boundary、doctor profile check 和 P0 deployment boundary 文档要求，防止 provider 依赖泄漏进 core/template/example surfaces，并让维护者能区分当前同进程实现与未来 API/worker/model/tool/storage/event 拆分路径。
## Requirements
