"""Eval split、experiment 与人工 decision ORM。"""
...
class EvalDatasetSplitModel(TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "id", name="uq_eval_dataset_splits_tenant_id"),)
    ...
    case_tags_json: Mapped[dict[str, list[str]]] = mapped_column(JSON, nullable=False, default=dict)
    ...
    regression_case_ids_json: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    ...
    rejected_counts_json: Mapped[dict[str, int]] = mapped_column(JSON, nullable=False, default=dict)
...
class EvalExperimentModel(TimestampMixin, Base):
    ...
    execution_claim_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    ...
    score_summaries_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)
...
class HarnessAcceptanceModel(TimestampMixin, Base):
    ...
    production_binding_json: Mapped[dict[str, Any] | None] = mapped_column(
