...
[总体规则]
    ...
    - 语言纪律：始终使用中文交流；创建或实质修改面向维护者的 Markdown、契约、指南，以及代码、测试、脚本、配置中的注释和说明时，默认使用项目主语言，不得无故把既有符合项目主语言的内容改成另一种语言；新引入的流程概念先确定项目主语言主称，首次出现可注明必要英文标识，后续统一使用主称，不为普通叙述保留英语黑话；命令、路径、标识符、协议字段、API 名称、CLI 参数、schema 字段、错误码和行业固定术语可保留英文，完成前做语言自检
    ...
    - 维护说明纪律：代码注释、docstring、测试说明、脚本说明、配置文件注释、配置示例说明和文档内注释遵循前述语言纪律，并按正常人类工程实践书写，用来帮助维护者理解维护意图、复杂控制流、数据形状、约束、边界、风险、设计取舍、兼容逻辑、迁移逻辑、测试夹具意图，以及项目代码规范明确要求说明的地方。避免逐行复述代码和 AI 式自述，注释里默认不用 emoji；不得为了少写注释、压行数、套“最小修正/最小补充”措辞或通过审查而删掉、弱化维护者需要的说明。API、worker、迁移、并发、安全边界、测试夹具、公开 seam 等非显然函数和复杂代码块，必须说明维护意图、边界或验证目的；配置、脚本和示例里的非显然约束也要说明维护边界、默认值来源、兼容风险或验证目的；简单样板可不写无信息注释。复杂关键设计可用块注释、步骤列表或图示承载；说明过长时沉淀到 ADR、design doc 或 README，代码里留短注释和链接。
