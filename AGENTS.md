...
[总体规则]
    ...
    - 代码注释和文档内注释按正常人类工程实践书写，用来帮助维护者理解维护意图、复杂控制流、数据形状、约束、边界、风险、设计取舍、兼容逻辑、迁移逻辑、测试夹具意图，以及项目代码规范明确要求说明的地方；默认使用项目主语言，必要的标识符、协议字段、API 名称保留英文。避免逐行复述代码和 AI 式自述，注释里默认不用 emoji；不得为了少写注释、压行数或通过审查而删掉维护者需要的说明。复杂关键设计可用块注释、步骤列表或图示承载；说明过长时沉淀到 ADR、design doc 或 README，代码里留短注释和链接。
